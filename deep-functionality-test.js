...
async function runDeepAudit() {
  ...
  try {
    ...
    const { generateCohortSpaces, getCohortSpaceId } = await import('./packages/core/dist/domain/cohort/cohort-spaces.js');
...
try {
  ...
  const filterMatch = spacesPage.match(/const spaceTypeFilters = \[([\s\S]*?)\];/);
  if (filterMatch) {
...
// 9. FINAL INTEGRATION TEST - END-TO-END FLOW
console.log('\n🧪 TESTING END-TO-END INTEGRATION...\n');

try {
  // Test the full user journey
  const onboardingFlow = fs.readFileSync('./apps/web/src/app/api/auth/complete-onboarding/route.ts', 'utf8');
  const hasAutoJoinCall = onboardingFlow.includes('/api/spaces/auto-join') || 
                         onboardingFlow.includes('auto-join') ||
                         onboardingFlow.includes('autoJoin');
  
  logTest('Onboarding triggers auto-join', hasAutoJoinCall,
    'New users automatically join cohort spaces');
    
  // Test space browsing integration
  const spacesPageImports = fs.readFileSync('./apps/web/src/app/(dashboard)/spaces/page.tsx', 'utf8');
  const hasProperImports = spacesPageImports.includes('UnifiedSpaceCard') &&
                          spacesPageImports.includes('authenticatedFetch');
  
  logTest('Spaces page has proper imports', hasProperImports,
  ...
  const hasWidgetSystem = spacePage.includes('HivePostsSurface') &&
                         spacePage.includes('HiveEventsSurface') &&
                         spacePage.includes('HiveMembersSurface');
  
  logTest('Individual space has complete widget system', hasWidgetSystem,
    'All widget components properly integrated');
    
} catch (error) {
  logTest('End-to-end integration', false, `Error: ${error.message}`, true);
}
