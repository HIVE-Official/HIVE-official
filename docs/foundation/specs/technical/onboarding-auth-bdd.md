...
> **Note:** Code paths referenced here describe planned locations in the rebuilt monorepo. Actual implementations will live under the new context directories (e.g., `apps/web/src/contexts/auth`, `apps/web/src/contexts/onboarding`, `packages/core/src/...`) once the slices are delivered.
...
**Planned Location:** `packages/core/src/domain/profile/aggregates/profile.aggregate.ts`
...
{
  profileId: ProfileId
  email: CampusEmail           // Campus-specific enforcement (v1: @buffalo.edu)
  handle: ProfileHandle        // Unique username (3-20 chars)
  userType: UserType           // student | alumni | faculty
  campusId: CampusId           // defaults to 'ub-buffalo' (additional campuses coming)
...
### 1. CampusEmail (`campus-email.value.ts`)

**Business Rule:** Email domain must belong to an approved campus (initial launch: @buffalo.edu)
...
Feature: Email Validation
  As the system
  I need to validate that only approved campus emails are used
  ...
  Scenario: Valid UB email
    ...
    Then the CampusEmail value object is created successfully
    And the email is normalized to lowercase

  Scenario: Invalid domain
    Given a user provides "student@gmail.com"
    When the email is validated
    Then validation fails with error "Only approved campus emails (e.g., @buffalo.edu) are allowed"
...
Feature: User Onboarding
  ...
  Scenario: Successful onboarding completion
    Given the user enters:
      ...
      | residentialSpace   | Ellicott Complex             |
      | consentGiven       | true                         |
    ...
    And the user is auto-joined to default spaces:
      ...
      - "New Students"
      - "Campus Updates"
      - The selected residential space ("Ellicott Complex")
...
**Planned Location:** `packages/core/src/domain/profile/events/profile-created.event.ts`
...
**Planned Location:** `packages/core/src/domain/profile/events/profile-onboarded.event.ts`
...
**Planned Location:** `packages/core/src/application/profile-onboarding.service.ts`
...
Then the service should:
  1. Validate email domain (CampusEmail value object)
...
**Planned Location:** `packages/auth-logic/src/hooks/use-auth.ts`
...
**Planned Location:** `apps/web/src/contexts/onboarding/api/complete-onboarding` (or equivalent route)
...
Invariant: Only @buffalo.edu emails can create profiles
  ...
  Then the email MUST belong to an approved campus domain (e.g., @buffalo.edu)
...
- [ ] CampusEmail.create() validates approved campus domains
- [ ] CampusEmail.create() rejects unapproved domains
...
- [x] ProfileOnboardingService.completeOnboarding() checks handle availability
- [ ] ProfileOnboardingService.completeOnboarding() initializes feed
- [x] ProfileOnboardingService.completeOnboarding() joins default spaces
- [x] ProfileOnboardingService handles duplicate handle error
...
7. **Verification Automation:** Current student leader verification is manual. What systems could automate approval later?
...
- **Domain Model (planned):** `packages/core/src/domain/profile/`
- **Application Services (planned):** `packages/core/src/application/profile-onboarding.service.ts`
- **API Routes (planned):** `apps/web/src/contexts/onboarding/api/` and `apps/web/src/contexts/auth/api/`
- **Client Hooks (planned):** `packages/auth-logic/src/hooks/use-auth.ts`
