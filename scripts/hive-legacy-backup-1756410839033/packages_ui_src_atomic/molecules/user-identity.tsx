...
export interface UserIdentityProps 
  extends React.HTMLAttributes<HTMLDivElement>,
    VariantProps<typeof userIdentityVariants> {
...
const StatusIndicator = ({ status, className }: { status: string; className?: string }) => {
  ...
  return (
    <div
      className={cn(
        // Base status dot
        'w-3 h-3 rounded-full border-2 border-[var(--hive-bg-primary)]',
        config.bg,
        // Position
...
// === MAIN COMPONENT ===
export const UserIdentity = React.forwardRef<HTMLDivElement, UserIdentityProps>(
  ({
    className,
    size,
    layout,
    ...
    return (
      <div
        ref={ref}
        className={cn(userIdentityVariants({ size, layout, interactive }), className)}
        onClick={onClick}
        ...
        >
          <div
            className={cn(
              ...
              `border border-[${borderComposition.colors.semantic.subtle}]`,
