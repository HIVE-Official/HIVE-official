    ...
    return (
      <div className="flex items-center justify-center h-96">
        <div className="text-center">
          <p className="text-lg text-[var(--hive-text-muted)] mb-4">Unable to load personal tools</p>
    ...
    <Card className={`h-full bg-gradient-to-br ${categoryColors[tool.category]} transition-all duration-200`}>
      <CardHeader className="pb-3">
        ...
        <div>
          ...
          <p className="text-xs text-[var(--hive-text-muted)] mt-1">{tool.description}</p>
        </div>
      </CardHeader>
...
function QuickStatCard({ label, value, icon, color }: QuickStatCardProps) {
  ...
  return (
    <div className="text-center">
      ...
      <div className="text-xs text-[var(--hive-text-muted)]">{label}</div>
    </div>
  );
}
