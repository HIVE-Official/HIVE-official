...
export function SocialToolsCard({
  ...
  const ToolButton: React.FC<{ tool: Tool }> = ({ tool }) => (
    ...
    >
      {/* New/Trending Badges */}
      {tool.isNew && (
        <Badge className="absolute -top-1 -right-1 bg-blue-500 text-[var(--hive-text-inverse)] text-xs px-1 py-0">
          NEW
        </Badge>
      )}
    ...
    >
      ...
      <div className="flex items-center justify-between mb-6">
        <div className="flex items-center gap-3">
          <div className="w-10 h-10 rounded-full bg-gradient-to-br from-orange-500 to-orange-600 flex items-center justify-center">
            <Wrench size={20} className="text-[var(--hive-text-inverse)]" />
          </div>
          <div>
      ...
      {isBuilder && (totalCreated > 0 || campusImpact > 0) && (
        <div className="flex items-center justify-between p-3 rounded-lg bg-gradient-to-r from-blue-500/10 to-blue-600/10 border border-blue-500/20 mb-6">
          <div className="flex items-center gap-3">
            <div className="w-8 h-8 rounded-full bg-blue-500 text-[var(--hive-text-inverse)] flex items-center justify-center">
              <Zap size={16} />
            </div>
            <div>
          ...
          <Badge className="bg-blue-500 text-[var(--hive-text-inverse)]">
            Top 5%
          </Badge>
        </div>
      )}
