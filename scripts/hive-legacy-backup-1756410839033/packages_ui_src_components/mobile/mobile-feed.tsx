    ...
    >
      {isRefreshing ? (
        <>
          ...
          <span className="text-sm text-[var(--hive-text-muted)]">Refreshing feed...</span>
        </>
      ) : canRefresh ? (
        <>
          <RefreshCw className="h-5 w-5 text-blue-500 mr-2" />
          <span className="text-sm text-[var(--hive-text-muted)]">Release to refresh</span>
        </>
      ) : (
        <>
          <ChevronDown 
          ...
          {hasMore && (
            <div className="flex items-center justify-center py-8">
              {isLoading ? (
                <>
                  <Loader2 className="h-5 w-5 animate-spin text-blue-500 mr-2" />
                  <span className="text-sm text-[var(--hive-text-muted)]">Loading more posts...</span>
                </>
              ) : (
                <ButtonEnhanced 
                  onClick={onLoadMore}
        ...
        <div className="absolute inset-0 bg-gradient-to-r from-blue-500/20 to-purple-500/20 z-10 flex items-center justify-end px-4">
          <div className="flex space-x-3">
            <ButtonEnhanced
              ...
              className="text-[var(--hive-text-inverse)] bg-blue-600/80 hover:bg-blue-600 touch-target-large"
            >
            ...
            <ButtonEnhanced
              ...
              className="text-[var(--hive-text-inverse)] bg-purple-600/80 hover:bg-purple-600 touch-target-large"
            >
      ...
      <CardContent className="p-4 space-y-3">
        ...
        <div className="flex items-center justify-between">
          <div className="flex items-center space-x-3">
            <div className="w-8 h-8 rounded-full bg-gradient-to-br from-blue-400 to-purple-500 flex items-center justify-center text-[var(--hive-text-inverse)] text-sm font-medium">
              {post.author.name.charAt(0).toUpperCase()}
            </div>
            <div>
              <div className="font-medium text-[var(--hive-text-inverse)] text-sm">{post.author.name}</div>
        ...
        <div className="space-y-3">
          ...
          {post.preview && (
            <div className="border border-gray-700 rounded-lg overflow-hidden">
              ...
              <div className="p-3">
                {post.preview.title && (
                  <h4 className="font-medium text-[var(--hive-text-inverse)] text-sm mb-1">{post.preview.title}</h4>
                )}
