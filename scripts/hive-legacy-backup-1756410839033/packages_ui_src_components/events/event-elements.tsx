    ...
    <div className={cn("space-y-2", className)}>
      ...
      <div className="relative">
        <input
          ...
          required={required}
          disabled={disabled}
          className="w-full p-2 border border-[var(--hive-border-default)] rounded-md focus:ring-2 focus:ring-amber-500 focus:border-amber-500"
        />
    ...
    <div className={cn("space-y-1", className)}>
      <label htmlFor={id} className="flex items-start gap-2 cursor-pointer">
        <input
          ...
          disabled={disabled}
          className="mt-0.5 rounded border-[var(--hive-border-default)] focus:ring-amber-500"
        />
        <div>
        ...
        {options.map((option) => (
          <label key={option.value} className="flex items-start gap-2 cursor-pointer">
            <input
              ...
              onChange={(e) => onChange?.(e.target.value)}
              disabled={disabled}
              className="mt-0.5 border-[var(--hive-border-default)] focus:ring-amber-500"
            />
            <div>
...
export function EventCardElement({ 
  ...
  return (
    <Card className={cn("p-4", className)}>
      ...
      {event.location && (
        <div className="flex items-center gap-2 text-sm text-[var(--hive-text-muted)] mb-3">
...
export function QRCodeElement({ 
  ...
  return (
    <div className={cn("text-center space-y-3", className)}>
      ...
      <div 
        className="mx-auto bg-[var(--hive-background-secondary)] border-2 border-dashed border-[var(--hive-border-default)] rounded-lg flex items-center justify-center"
      ...
      {onScan && (
        <Button size="sm" onClick={() => onScan(data)}>
          Scan Code
        </Button>
      )}
    </div>
        ...
        {attendees.map((attendee) => (
          <div key={attendee.id} className="flex items-center justify-between p-2 border border-gray-200 rounded-lg">
            <div className="flex-1">
              ...
              <p className="text-sm text-[var(--hive-text-muted)]">{attendee.email}</p>
...
export function CalendarViewElement({ 
  ...
  return (
    <Card className={cn("p-4", mobileClasses.card, className)}>
      ...
      <div className="grid grid-cols-7 gap-1 mb-2">
        {(isMobile ? ['S', 'M', 'T', 'W', 'T', 'F', 'S'] : ['Sun', 'Mon', 'Tue', 'Wed', 'Thu', 'Fri', 'Sat']).map((day, index) => (
          <div key={index} className={cn("p-2 text-center font-medium text-[var(--hive-text-muted)]", mobileClasses.text)}>
            {day}
          </div>
        ))}
      </div>
            ...
            return (
              ...
              >
                <div className="flex gap-3">
                  <Icon className="w-5 h-5 text-[var(--hive-text-muted)] mt-0.5 flex-shrink-0" />
                  <div className="flex-1">
                    ...
                    <p className="text-sm text-[var(--hive-text-muted)] mt-1">{notification.message}</p>
                  </div>
                </div>
              </div>
            );
  ...
  return (
    <Card className={cn("p-4", className)}>
      <div className="flex items-center justify-between mb-4">
        ...
        <TrendingUp className="w-5 h-5 text-[var(--hive-text-muted)]" />
      </div>
        ...
        <div className="h-32 flex items-end justify-between border-b border-gray-200">
          {data.map((item, index) => (
            <div key={index} className="flex flex-col items-center space-y-2">
              ...
              />
              <span className="text-xs text-[var(--hive-text-muted)] -rotate-45 transform origin-left">
                {item.label}
              </span>
            </div>
          ))}
        ...
        <div key={question.id} className="space-y-2">
          ...
          {question.type === 'select' && question.options && (
            <select
              value={responses[question.id] || ''}
              onChange={(e) => updateResponse(question.id, e.target.value)}
              className="w-full p-2 border border-[var(--hive-border-default)] rounded-md focus:ring-2 focus:ring-amber-500"
            >
  ...
  return (
    <div className={cn("space-y-3", className)}>
      ...
      {availableTags.length > 0 && (
        <div className="space-y-2">
          <p className="text-sm text-[var(--hive-text-muted)]">Suggested tags:</p>
          <div className="flex flex-wrap gap-2">
            {availableTags
      ...
      <div className="space-y-2">
        ...
        {showReasonInput && (
          <div className="space-y-2">
            ...
            />
            <div className="flex gap-2">
              <Button size="sm" onClick={submitStatusChange}>
                Confirm
              </Button>
              <Button variant="secondary" size="sm" onClick={() => setShowReasonInput(false)}>
                Cancel
              </Button>
            </div>
          </div>
        ...
        <div className="space-y-2">
          ...
          <div className="space-y-2 max-h-32 overflow-y-auto">
            {statusHistory.map((entry, index) => (
              <div key={index} className="text-sm text-[var(--hive-text-muted)] p-2 bg-gray-50 rounded">
