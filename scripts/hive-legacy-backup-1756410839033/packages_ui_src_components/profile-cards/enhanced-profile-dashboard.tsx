...
const ComingSoonCard: React.FC<{ title: string; description: string; icon: React.ElementType }> = ({ 
  ...
  <Card className="h-full flex flex-col items-center justify-center text-center p-6">
    ...
    <Badge variant="active-tag" className="bg-[var(--hive-brand-secondary)]/20 text-[var(--hive-brand-secondary)] border-hive-gold/30">
...
const PersonalToolsCard: React.FC<{ 
  ...
  <Card className="h-full p-6">
    <div className="flex items-center justify-between mb-4">
      ...
      <Badge variant="course-tag">{tools.length}/9</Badge>
    </div>
    ...
    <div className="grid grid-cols-3 gap-3 mb-4">
      {tools.slice(0, 9).map((tool) => (
        ...
        >
          ...
          {tool.isForked && (
            <div className="mt-1">
              <Badge variant="tool-tag" className="text-xs px-1 py-0">Fork</Badge>
            </div>
          )}
        </motion.div>
      ))}
