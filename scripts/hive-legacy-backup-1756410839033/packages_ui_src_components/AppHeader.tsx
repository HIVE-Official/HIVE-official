...
>(({ className, placeholder = "Search...", variant = 'chip', shortcut = "⌘K", onSearchClick, ...props }, ref) => {
    ...
    return (
      <MotionDiv
        ...
        {...motionProps}
      >
        <Search className="w-4 h-4 text-[var(--hive-text-inverse)]/40 flex-shrink-0" />
        <span className="flex-1 text-[var(--hive-text-inverse)]/60 text-sm font-medium">{placeholder}</span>
        <div className="flex items-center gap-1 text-xs text-[var(--hive-text-inverse)]/40">
...
>(({ className, isOpen = false, variant = 'chip', ...props }, ref) => {
    ...
    return (
      <MotionButton
        ref={ref}
        className={cn(
          'inline-flex items-center justify-center text-[var(--hive-text-inverse)]/60 hover:text-accent transition-colors md:hidden',
          getVariantStyles(),
          className
        )}
        whileHover={{ 
...
>(({ className, count = 0, variant = 'chip', showDot = false, ...props }, ref) => {
    ...
    return (
      <MotionButton
        ref={ref}
        className={cn(
          'relative inline-flex items-center justify-center text-[var(--hive-text-inverse)]/60 hover:text-accent transition-colors',
          getVariantStyles(),
          className
        )}
        whileHover={{ 
        ...
        {(count > 0 || showDot) && (
          <MotionDiv
            className={cn(
              ...
              count > 0 
                ? "min-w-[1.25rem] h-5 text-xs font-semibold text-[var(--hive-text-primary)] bg-accent rounded-full" 
