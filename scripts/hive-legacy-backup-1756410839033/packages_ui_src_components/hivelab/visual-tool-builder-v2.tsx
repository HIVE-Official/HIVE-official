...
function CanvasComponent({ component, isSelected, onSelect, onUpdate, onDelete }: {
  ...
  const renderComponent = () => {
    switch (component.type) {
      ...
      case 'input':
        return <Input placeholder={component.placeholder || 'Enter text...'} className="w-full" />;
      case 'textarea':
        return <Textarea placeholder={component.placeholder || 'Enter long text...'} className="w-full" />;
      case 'button':
        return (
          <Button variant={component.style?.variant || 'default'} size={component.style?.size || 'md'}>
            {component.label || 'Button'}
          </Button>
        );
      case 'number':
        return <Input type="number" placeholder="0" className="w-full" />;
      case 'checkbox':
        return (
          <label className="flex items-center gap-2">
            <input type="checkbox" className="rounded border-[var(--hive-border-default)]" />
            <span>{component.label || 'Checkbox option'}</span>
          </label>
        );
      case 'select':
        return (
          <select className="w-full p-2 border border-[var(--hive-border-default)] rounded-md">
      ...
      case 'divider':
        return <hr className="border-[var(--hive-border-default)] w-full" />;
      case 'card':
        return (
          <Card className="p-4 min-h-[80px] border-dashed border-[var(--hive-border-default)]">
            <p className="text-gray-500 text-center">Card content goes here</p>
          </Card>
        );
      default:
        return <div className="p-2 bg-[var(--hive-background-secondary)] rounded text-sm">{component.type}</div>;
    }
  };
  ...
  return (
    ...
    >
      ...
      >
        <Grip className="w-3 h-3 text-[var(--hive-text-inverse)]" />
      </div>
      ...
      >
        <Trash2 className="w-3 h-3 text-[var(--hive-text-inverse)]" />
      </button>
      ...
      {(['button', 'heading'].includes(component.type)) && (
        <div>
          ...
          <select
            ...
            className="w-full p-2 border border-[var(--hive-border-default)] rounded-md"
          >
        ...
        <div className="flex-1 flex flex-col">
          ...
          <div className="bg-white border-b border-gray-200 p-4 flex items-center justify-between">
            ...
            <div className="flex items-center gap-2">
              ...
              <Button onClick={saveTool} size="sm">
          ...
          <div className="flex-1 p-8 overflow-auto">
            <div
              ...
              className={cn(
                ...
                isOver ? "border-amber-400 bg-amber-50" : "border-[var(--hive-border-default)]",
