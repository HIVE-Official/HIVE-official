...
import { ButtonEnhanced, Card, useUnifiedAuth } from "@hive/ui";
  ...
  return (
    <div className="p-4 bg-hive-background-secondary rounded-lg">
      ...
      <div className="flex gap-2">
        {!membershipRole && (
          <ButtonEnhanced onClick={() => onJoin?.(space.id)} size="sm">Join</ButtonEnhanced>
        )}
        {membershipRole && (
          <ButtonEnhanced onClick={() => onLeave?.(space.id)} size="sm" variant="outline">Leave</ButtonEnhanced>
        )}
        <ButtonEnhanced onClick={() => onMessage?.(space.id)} size="sm" variant="outline">Message</ButtonEnhanced>
        {(membershipRole === 'admin' || membershipRole === 'owner') && (
          <ButtonEnhanced onClick={() => onSettings?.(space.id)} size="sm" variant="outline">Settings</ButtonEnhanced>
        )}
      </div>
    </div>
  );
