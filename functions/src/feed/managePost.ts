...
async function assertIsBuilder(uid: string, spaceId: string): Promise<void> {
  ...
  if (!memberDoc.exists || memberData?.role !== "builder") {
    throw new functions.https.HttpsError(
      "permission-denied",
      "User is not a builder of this space."
    );
  }
}
...
export const editPost = createHttpsFunction<EditPostData>(
  async (data: EditPostData, context: FunctionContext) => {
...
export const deletePost = createHttpsFunction<DeletePostData>(
  async (data: DeletePostData, context: FunctionContext) => {
    const uid = context.auth?.uid;
