...
describe("Spaces Security Rules", () => {
  ...
  describe("Space Creation and Deletion", () => {
    test("authenticated users cannot create spaces directly", async () => {
      const alice = authenticatedContext("alice");
      await assertFails(
        setDoc(doc(alice.firestore(), "spaces", "new-space"), {
          name: "New Space",
  ...
  describe("Space Updates", () => {
    ...
    test("non-members cannot update spaces", async () => {
      ...
      await assertFails(
        updateDoc(doc(charlie.firestore(), "spaces", "space1"), {
          description: "Updated description",
        })
      );
    });
  ...
  describe("Member Management", () => {
    test("authenticated users can read member lists", async () => {
      ...
      await assertSucceeds(
        getDocs(collection(bob.firestore(), "spaces", "space1", "members"))
      );
    });
    ...
    test("users cannot join spaces directly", async () => {
      ...
      const alice = authenticatedContext("alice");
      await assertFails(
        setDoc(doc(alice.firestore(), "spaces", "space1", "members", "alice"), {
          uid: "alice",
          role: "member",
          joinedAt: new Date(),
        })
    ...
    test("users cannot delete other members", async () => {
      ...
      const bob = authenticatedContext("bob");
      await assertFails(
        deleteDoc(doc(bob.firestore(), "spaces", "space1", "members", "alice"))
      );
    });
    ...
    test("users cannot update other members roles", async () => {
      ...
      await assertFails(
        updateDoc(
          doc(bob.firestore(), "spaces", "space1", "members", "alice"),
          {
            role: "requested_builder",
          }
        )
  ...
  describe("Posts Management", () => {
    test("space members can read posts", async () => {
      ...
      await assertSucceeds(
        getDocs(collection(alice.firestore(), "spaces", "space1", "posts"))
      );
    });
    ...
    test("non-members cannot read posts", async () => {
      ...
      await assertFails(
        getDocs(collection(bob.firestore(), "spaces", "space1", "posts"))
      );
    });
    ...
    test("non-members cannot create posts", async () => {
      ...
      const bob = authenticatedContext("bob");
      await assertFails(
        setDoc(doc(bob.firestore(), "spaces", "space1", "posts", "new-post"), {
          authorId: "bob",
          content: "New post content",
          createdAt: new Date(),
          updatedAt: new Date(),
    ...
    test("post authors can update their own posts", async () => {
      ...
      await assertSucceeds(
        updateDoc(
          doc(alice.firestore(), "spaces", "space1", "posts", "post1"),
          {
    ...
    test("other users cannot update posts", async () => {
      ...
      const bob = authenticatedContext("bob");
      await assertFails(
        updateDoc(doc(bob.firestore(), "spaces", "space1", "posts", "post1"), {
    ...
    test("post authors can delete their own posts", async () => {
      ...
      const alice = authenticatedContext("alice");
      await assertSucceeds(
        deleteDoc(doc(alice.firestore(), "spaces", "space1", "posts", "post1"))
      );
    });
...
describe("Schools Security Rules", () => {
  ...
  test("non-admin users cannot write to schools", async () => {
    const alice = authenticatedContext("alice");
    await assertFails(
      setDoc(doc(alice.firestore(), "schools", "new-school"), {
        name: "Test School",
        domain: "test.edu",
        status: "waitlist",
        waitlistCount: 0,
  ...
  test("waitlist entries are completely protected", async () => {
    ...
    await assertFails(
      getDocs(
        collection(alice.firestore(), "schools", "school1", "waitlist_entries")
      )
    );
    await assertFails(
      setDoc(
