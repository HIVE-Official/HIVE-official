...
function PaletteItem({ element }: { element: typeof ELEMENT_PALETTE[0] }) {
  const { attributes, listeners, setNodeRef, transform, isDragging } = useDraggable({
    ...
    data: { type: element.type, fromPalette: true },
  });
  ...
  return (
    <div
      ...
      {...listeners}
      {...attributes}
      className="p-3 bg-zinc-800 border border-zinc-700 rounded-lg cursor-grab hover:bg-zinc-750 transition-colors group"
    >
    ...
    >
      <div className="h-screen flex bg-[#0A0A0A]">
        ...
        <div className="flex-1 flex flex-col">
          ...
          <div className="flex-1 p-6 overflow-auto">
            <div
              ref={setCanvasRef}
              className="min-h-full bg-zinc-950 rounded-lg border-2 border-dashed border-zinc-700 p-6"
            >
              {elements.length === 0 ? (
                <div className="text-center text-zinc-500 py-16">
                  <Square className="w-12 h-12 mx-auto mb-4" />
                  <h3 className="text-lg font-medium mb-2">Start building your tool</h3>
                  <p>Drag elements from the left panel to get started</p>
                </div>
              ) : (
      ...
      <DragOverlay>
        {activeId && activeId.startsWith('palette-') ? (
          <div className="p-3 bg-zinc-800 border border-zinc-700 rounded-lg opacity-75">
            <div className="w-8 h-8 bg-blue-500 rounded-lg mx-auto" />
          </div>
        ) : null}
      </DragOverlay>
    </DndContext>
