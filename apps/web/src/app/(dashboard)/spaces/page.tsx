...
import { Input, PageContainer, Button } from "@hive/ui";
import { Search, AlertCircle, Compass, Plus } from "lucide-react";
// import { SpaceFilterPills } from "../../../app/spaces/components/space-filter-pills"; // Component removed
import { type SpaceType, type Space } from "@hive/core";
import { HiveSpaceCard as SpaceCard } from "@hive/ui";
...
async function fetchSpaces(
  ...
  try {
    ...
    if (sessionJson) {
      ...
      // For development, use test token; for production, use actual token
      headers.Authorization = `Bearer test-token`;
    } else {
      headers.Authorization = `Bearer test-token`; // Fallback for development
    }
  } catch (error) {
...
export default function SpacesDiscoveryPage() {
  ...
  return (
    <PageContainer
      ...
      subtitle="Find communities, join conversations, and build with others at your campus."
      breadcrumbs={[
    ...
    >
      ...
      {/* Filter Pills - Real space types */}
      <div className="mb-8">
        <div className="flex gap-2 flex-wrap">
          {(['all', 'student_organizations', 'campus_living', 'fraternity_and_sorority', 'university_organizations', 'hive_exclusive'] as const).map((filter) => (
            <button
              key={filter}
            ...
            >
              {filter === 'all' ? 'All Spaces' : 
               filter === 'student_organizations' ? 'Student Orgs' :
               filter === 'campus_living' ? 'Campus Living' :
               filter === 'fraternity_and_sorority' ? 'Greek Life' :
               filter === 'university_organizations' ? 'University' :
               'HIVE Exclusive'}
            </button>
          ))}
        </div>
