...
import { Grid, Button, Input, Card, Badge, useToast } from "@hive/ui";
import {
  Search,
  Plus,
  Users,
...
export default function SpacesDirectoryPage() {
  ...
  const [panicReliefSpaces, setPanicReliefSpaces] = useState<
    SpaceRecommendation[]
  >([]);
  ...
  const loadRecommendations = useCallback(async () => {
    ...
    try {
      ...
      const recommended = await api.get<{
        panicRelief: SpaceRecommendation[];
        whereYourFriendsAre: SpaceRecommendation[];
        insiderAccess: SpaceRecommendation[];
        recommendations: SpaceRecommendation[];
      }>("/api/spaces/recommended");

      setPanicReliefSpaces(recommended.panicRelief || []);
      setFriendSpaces(recommended.whereYourFriendsAre || []);
      setInsiderSpaces(recommended.insiderAccess || []);

      // Load category spaces
      if (selectedCategory !== "all") {
        const categoryResponse = await api.get<{ spaces: SpaceRecommendation[] }>("/api/spaces", {
          params: {
  ...
  }, [user, selectedCategory, showToast]);
  ...
  const handleSearch = async () => {
    ...
    try {
      setLoading(true);
      const searchResponse = await api.get<{ spaces: SpaceRecommendation[] }>("/api/spaces/search", {
        params: {
          q: searchQuery,
          limit: 20,
        },
      ...
      const spaces = searchResponse.spaces || [];
              ...
              <div className="flex items-center gap-3 mb-4">
                ...
                <div>
                  ...
                  <p className="text-sm text-gray-400">
                    Spaces that solve immediate anxieties
                  </p>
                </div>
              </div>
            ...
            <section>
              <div className="flex items-center gap-3 mb-4">
                ...
                <div>
                  ...
                  <p className="text-sm text-gray-400">
                    Spaces with existing connections
                  </p>
                </div>
              </div>
              <Grid columns={1} responsive="adaptive" gap={4}>
              ...
              <div className="flex items-center gap-3 mb-4">
                <div>
                  <h2 className="text-xl font-bold text-white">
                    {
                      SPACE_CATEGORIES[
                        selectedCategory as keyof typeof SPACE_CATEGORIES
                      ]?.label
                    }{" "}
                    Spaces
                  </h2>
...
function SpaceCard({
  ...
  return (
    ...
    >
      {/* Banner */}
      <div className="relative h-24 overflow-hidden bg-gradient-to-br from-hive-gold/20 to-purple-600/20">
        {space.bannerImage && (
          <Image
      ...
      <div className="p-4">
        <div className="flex items-start justify-between mb-2">
          ...
          {/* Activity Level */}
          <div
            className={`w-2 h-2 rounded-full ${activityColors[space.activityLevel]}`}
          />
        </div>
        ...
        {space.joinToActiveRate >= 0.7 && (
          <div className="mt-2 pt-2 border-t border-gray-800">
            <p className="text-xs text-green-400">
              ✓ {Math.round(space.joinToActiveRate * 100)}% become active
              members
            </p>
          </div>
        )}
