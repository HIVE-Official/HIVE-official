...
function VerifyPageContent() {
  ...
  const getIcon = () => {
    switch (state.status) {
      case 'loading':
      case 'verifying':
        return <Loader2 className="h-16 w-16 text-[var(--hive-gold)] animate-spin" />;
      case 'success':
        return <CheckCircle className="h-16 w-16 text-[var(--hive-gold)]" />;
      case 'error':
        return <XCircle className="h-16 w-16 text-[var(--hive-gold)]" />;
    }
  };
        ...
        {state.status === 'error' && (
          <div className="space-y-3">
            <button
              ...
              className="w-full bg-[var(--hive-brand-primary)] text-[var(--hive-text-inverse)] py-3 px-4 rounded-lg hover:opacity-90 transition-opacity"
            >
              Return to Login
            </button>
    ...
    <Suspense 
      fallback={
        <div className="min-h-screen bg-[var(--hive-background-primary)] flex items-center justify-center">
          <Loader2 className="h-12 w-12 text-[var(--hive-gold)] animate-spin" />
        </div>
      }
    >
      <VerifyPageContent />
