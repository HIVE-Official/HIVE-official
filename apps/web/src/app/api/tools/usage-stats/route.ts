...
export const POST = withAuth(async (request: NextRequest, authContext) => {
  try {
    ...
    if (!toolId || !action) {
      return NextResponse.json(_ApiResponseHelper.error("Invalid request. Must specify toolId and action", _ErrorCodes.INVALID_INPUT), { status: HttpStatus.BAD_REQUEST });
    }
