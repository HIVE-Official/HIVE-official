...
export const GET = withAuthAndErrors(async (context) => {
  ...
  try {
    ...
    logger.error('Error fetching system health', { error: error instanceof Error ? error.message : String(error), userId: auth.userId });
...
async function getDatabaseHealth(since: Date) {
  try {
    ...
    logger.error('Error getting database health', { error: error instanceof Error ? error.message : String(error) });
    return {
...
async function getAuthenticationHealth(since: Date) {
  try {
    ...
    logger.error('Error getting auth health', { error: error instanceof Error ? error.message : String(error) });
...
async function getApiHealth(since: Date) {
  try {
    ...
    logger.error('Error getting API health', { error: error instanceof Error ? error.message : String(error) });
...
async function getCacheHealth(since: Date) {
  try {
    ...
    logger.error('Error getting cache health', { error: error instanceof Error ? error.message : String(error) });
