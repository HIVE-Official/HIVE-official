 ...
 */
export async function GET(request: NextRequest, { params }: PublicProfileParams) {
  try {
    const { userId } = params;
    ...
    // Optional: Check if requesting user is authenticated (for privacy controls)
    ...
    if (authHeader && authHeader.startsWith('Bearer ')) {
      ...
      if (token.startsWith('dev_token_')) {
        ...
        // Return mock development profile if requesting dev user
...
async function getPublicSpaceMemberships(userId: string) {
  try {
    ...
    console.error('Error fetching public space memberships:', error);
    return [];
  }
}
...
async function getPublicActivity(userId: string) {
  try {
    ...
    console.log('Activity data not available yet');
    return [];
  }
}