...
import { COLLECTIONS, UserIntegrations } from '@/lib/firebase/collections/firebase-collections';
  ...
  try {
    ...
    const integrationStatus = {
      google: {
        ...
        expired: calendar.google?.expiresAt && (calendar.google.expiresAt?.toDate ? calendar.google.expiresAt.toDate() : new Date(calendar.google.expiresAt)) < now,
        lastSync: data.external?.lastSync?.google?.toDate() || null
      },
      outlook: {
        connected: !!(calendar.outlook?.accessToken),
        expired: calendar.outlook?.expiresAt && (calendar.outlook.expiresAt?.toDate ? calendar.outlook.expiresAt.toDate() : new Date(calendar.outlook.expiresAt)) < now,
...
export const PUT = withAuth(async (request: NextRequest, { userId }) => {
  try {
    ...
    if (providerConfig.expiresAt && (providerConfig.expiresAt?.toDate ? providerConfig.expiresAt.toDate() : new Date(providerConfig.expiresAt)) < new Date()) {
