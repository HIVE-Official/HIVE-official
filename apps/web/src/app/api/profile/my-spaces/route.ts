...
export async function GET(request: NextRequest) {
  try {
    ...
    const allSpacesSnapshot = await dbAdmin.collectionGroup('members')
      .where('userId', '==', userId)
      .get();
    ...
    const spaceIds = allSpacesSnapshot.docs.map(doc => {
    ...
    const spacePromises = spaceIds.map(async (spaceId) => {
      ...
      const membershipDoc = allSpacesSnapshot.docs.find(doc => 
        doc.ref.parent.parent?.id === spaceId
      );
    ...
    const validSpaces = spacesData.filter(space => space !== null);
    ...
    const spacesByType = validSpaces.reduce((acc, space) => {
