...
describe('CalendarPage', () => {
  ...
  describe('View Toggle', () => {
    it('switches between different views', () => {
      ...
      expect(listButton).toHaveClass('bg-[#FFD700]', 'text-[#0A0A0A]');

      // Switch to month view
      fireEvent.click(monthButton);
      expect(monthButton).toHaveClass('bg-[#FFD700]', 'text-[#0A0A0A]');

      // Switch to week view
      fireEvent.click(weekButton);
      expect(weekButton).toHaveClass('bg-[#FFD700]', 'text-[#0A0A0A]');
    });
    ...
    it('maintains view selection state', () => {
      ...
      expect(monthButton).toHaveClass('bg-[#FFD700]', 'text-[#0A0A0A]');
      expect(screen.getByText('List')).not.toHaveClass('bg-[#FFD700]');
    });
  });
