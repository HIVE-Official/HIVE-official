...
class FeedConfigurationManager {
  ...
  private async _doInitialize(): Promise<FeedConfiguration> {
    try {
      ...
      this.unsubscribeFirestore = onSnapshot(
        ...
        (error) => {
          logger.error('Error listening to feed configuration', { error });
      ...
      logger.error('Failed to initialize feed configuration', { error });
      this.config = DEFAULT_FEED_CONFIG;
      return this.config;
    }
  }
  ...
  private async saveConfig(config: FeedConfiguration): Promise<void> {
    try {
      ...
      logger.error('Failed to save feed configuration', { error });
      throw error;
    }
  }
  ...
  private notifyListeners(): void {
    ...
    this.listeners.forEach(listener => {
      try {
        ...
        logger.error('Error in feed config listener', { error });
      }
    });
  }
