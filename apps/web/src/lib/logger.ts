...
interface LogContext {
  ...
  // connectionCount already defined above at line 153
  connectionId?: string;
  count?: number;
  created?: boolean;
  currentCount?: number;
  ...
  // id already defined above at line 25
  ...
  result?: any;
  ritualCount?: number;
  // ritualId already defined above at line 166
  sourceId?: string;
  sourceType?: string;
  spaceConfig?: any;
...
class Logger {
  ...
  error(message: string, contextOrError?: LogContext | Error | unknown, error?: any): void {
    // Support both signatures: error(msg, context, error) and error(msg, error)
    const context = contextOrError instanceof Error ? undefined : contextOrError as LogContext | undefined;
  ...
  performanceStart(action: string): () => void {
    const startTime = performance.now();
    
    return () => {
      const duration = performance.now() - startTime;
      this.debug(`Performance: ${action} completed in ${duration.toFixed(2)}ms`, {
        action: 'performance',
        metadata: { operation: action, duration },
      });
    };
  }
}
