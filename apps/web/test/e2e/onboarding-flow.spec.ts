...
test.describe("Onboarding Flow", () => {
  test("should allow a new user to complete the full onboarding process", async ({
    ...
    await expect(page).toHaveTitle(/HIVE/); // Or whatever the title is
    ...
    await page.fill('input[id="handle"]', "test-user");
    // Wait for handle validation if any
    await page.waitForTimeout(600); // Wait for debounce
    await page.click('button:has-text("Continue")');
    await page.waitForURL("**/onboarding/2");

    // Step 2a: Leader Question (No path)
    await expect(page.locator("h1")).toContainText("Are you a student leader?");
    await page.click('button:has-text("No, not right now")');
    await page.click('button:has-text("Continue")');
    ...
    await page.selectOption('select[id="major"]', {
      label: "Computer Science",
    });
    await page.selectOption('select[id="gradYear"]', { label: "2025" });
    await page.check('input[id="isFirstYear"]');
    await page.click('button:has-text("Continue")');
    await page.waitForURL("**/onboarding/5");

    // Step 5: Avatar (Skip)
    await expect(page.locator("h1")).toContainText("Add a Profile Picture");
    await page.click('button:has-text("Skip for Now")');
    await page.waitForURL("**/onboarding/6");

    // Step 6: Interests
    await expect(page.locator("h1")).toContainText("What are you into?");
    ...
    await page.waitForURL("**/onboarding/7"); // Should be complete page
  ...
  test("complete onboarding flow - happy path", async ({ page }) => {
    ...
    await expect(
      page.locator('[data-testid="handle-available"]')
    ).toBeVisible();
    ...
    await expect(page.locator('[data-testid="welcome-mat"]')).toContainText(
