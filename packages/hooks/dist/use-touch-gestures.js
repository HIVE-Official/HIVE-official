...
function useTouchGestures(options = {}, callbacks = {}) {
    ...
    // State
    const [pullToRefreshState, setPullToRefreshState] = (0, react_1.useState)({
        distance: 0,
        isTriggered: false,
        isRefreshing: false
    });
    const [touchState, setTouchState] = (0, react_1.useState)({
        isActive: false,
        startPosition: null,
    ...
    const touchCurrentRef = (0, react_1.useRef)(null);
    const longPressTimerRef = (0, react_1.useRef)(null);
    const lastTapRef = (0, react_1.useRef)(null);
    const lastTapTimeRef = (0, react_1.useRef)(0);
    const initialScrollTopRef = (0, react_1.useRef)(0);
    const log = (0, react_1.useCallback)((message, ...args) => {
        if (debug) {
            console.log(`[TouchGestures] ${message}`, ...args);
        }
    }, [debug]);
    const triggerHaptic = (0, react_1.useCallback)((type = 'light') => {
    ...
    const getTouchPosition = (0, react_1.useCallback)((e) => {
    ...
    const calculateDistance = (0, react_1.useCallback)((start, end) => {
        const dx = end.x - start.x;
        const dy = end.y - start.y;
        return Math.sqrt(dx * dx + dy * dy);
    }, []);
    const calculateVelocity = (0, react_1.useCallback)((start, end) => {
        const distance = calculateDistance(start, end);
        const time = end.timestamp - start.timestamp;
        return time > 0 ? distance / time : 0;
    }, [calculateDistance]);
    const getSwipeDirection = (0, react_1.useCallback)((start, end) => {
        const dx = end.x - start.x;
        const dy = end.y - start.y;
    ...
    const handleTouchCancel = (0, react_1.useCallback)((_e) => {
