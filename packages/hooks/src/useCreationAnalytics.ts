...
import { 
  ...
  hashUserId
} from '@hive/core/domain/analytics/creation'
...
export const useCreationAnalytics = (options: UseCreationAnalyticsOptions = {}) => {
  const { user } = useAuth()
  const {
    toolId,
    spaceId,
  ...
  // Context tracking
  const [currentContext, setCurrentContext] = useState<CreationAnalyticsContext>({
    toolId,
    spaceId,
  })
  ...
  // Update context
  const updateContext = useCallback((context: Partial<CreationAnalyticsContext>) => {
    setCurrentContext(prev => ({ ...prev, ...context }))
  }, [])

  // Builder session tracking
  const startBuilderSession = useCallback((toolId: string, toolName?: string) => {
  ...
  const endBuilderSession = useCallback((exitReason: 'save' | 'abandon' | 'publish' | 'share' = 'abandon') => {
  ...
  const trackCanvasModeChanged = useCallback((mode: 'design' | 'preview' | 'code') => {
    trackEvent('canvas_mode_changed', { mode, previousMode: currentContext.toolStatus })
  }, [trackEvent, currentContext])

  const trackDeviceModeChanged = useCallback((deviceMode: 'desktop' | 'tablet' | 'mobile') => {
    trackEvent('device_mode_changed', { deviceMode })
  }, [trackEvent])

  const trackElementLibrarySearched = useCallback((searchQuery: string, resultsCount: number) => {
