...
import { validateEmailDomain } from '@hive/core/domain/auth/emailLink';
...
describe('Auth Logic', () => {
  ...
  describe('Handle Check API', () => {
    it('should return available for new handle', async () => {
      mockFetch.mockResolvedValueOnce({
        ok: true,
        json: async () => ({ available: true, handle: 'newhandle' }),
      });
    ...
    it('should return unavailable for taken handle', async () => {
      mockFetch.mockResolvedValueOnce({
        ok: true,
        json: async () => ({ available: false, handle: 'takenhandle' }),
      });
    ...
    it('should validate handle format', async () => {
      mockFetch.mockResolvedValueOnce({
        ok: false,
        status: 400,
        json: async () => ({ error: 'Invalid handle format' }),
      });
    ...
    it('should normalize handle to lowercase', async () => {
      mockFetch.mockResolvedValueOnce({
        ok: true,
        json: async () => ({ available: true, handle: 'testhandle' }),
      });
  ...
  describe('Onboarding API', () => {
    ...
    it('should reject onboarding without consent', async () => {
      mockFetch.mockResolvedValueOnce({
        ok: false,
        status: 400,
        json: async () => ({ error: 'Consent must be given' }),
      });
