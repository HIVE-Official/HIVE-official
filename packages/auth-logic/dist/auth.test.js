...
const validateEmailDomain = vi.fn((email, schoolDomain) => {
...
describe("Auth Logic", () => {
    ...
    describe("validateEmailDomain", () => {
        it("should validate matching email domains", () => {
            ...
            expect(validateEmailDomain("test@university.edu", "university.edu")).toBe(true);
        });
        it("should reject non-matching email domains", () => {
            expect(validateEmailDomain("user@wrong.edu", "correct.edu")).toBe(false);
            expect(validateEmailDomain("test@gmail.com", "university.edu")).toBe(false);
        });
    ...
    describe("Magic Link API", () => {
        ...
        it("should reject invalid email format", async () => {
            const mockResponse = { error: "Invalid email address" };
            mockFetch.mockResolvedValueOnce({
                ok: false,
                status: 400,
                json: () => Promise.resolve(mockResponse),
    ...
    describe("Handle Check API", () => {
        ...
        it("should validate handle format", async () => {
            const mockResponse = { error: "Invalid handle format" };
            mockFetch.mockResolvedValueOnce({
                ok: false,
                status: 400,
                json: () => Promise.resolve(mockResponse),
    ...
    describe("Onboarding API", () => {
        ...
        it("should reject onboarding without consent", async () => {
            const mockResponse = { error: "Consent must be given" };
            mockFetch.mockResolvedValueOnce({
                ok: false,
                status: 400,
                json: () => Promise.resolve(mockResponse),
        ...
        it("should reject invalid handle formats", async () => {
            ...
            const response = await fetch("/api/auth/complete-onboarding", {
                ...
                body: JSON.stringify({
                    fullName: "Test User",
                    major: "Computer Science",
                    handle: "invalid-handle!", // Invalid characters builderOptIn: false          consentGiven: true,
                }),
            });
            expect(response.ok).toBe(false);
            expect(response.status).toBe(400);
        ...
        it("should reject taken handles", async () => {
            const mockResponse = { error: "Handle is already taken" };
            mockFetch.mockResolvedValueOnce({
                ok: false,
                status: 409,
                json: () => Promise.resolve(mockResponse),
