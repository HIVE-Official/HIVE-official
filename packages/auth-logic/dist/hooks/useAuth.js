...
import { SessionStatus, useSessionManager } from '../session';
const initialState = {
...
export const useAuth = () => {
    ...
    // Initialize session
    const initSession = useCallback(async (sessionData) => {
        try {
            ...
            const authError = _error instanceof Error
                ? createAuthError(AuthErrorCode.INVALID_SESSION, { error: _error.message })
    ...
    const signOut = useCallback(async () => {
        try {
            ...
            const authError = _error instanceof Error
                ? createAuthError(AuthErrorCode.UNKNOWN_ERROR, { error: _error.message })
                : createAuthError(AuthErrorCode.UNKNOWN_ERROR);
            setState(prev => ({
                ...prev,
                error: authError
    ...
    return {
        // State status: state.status    session: state.session,
        error: state.error,
        // Methods
        initSession,
        signOut,
        clearError,
        // Computed isAuthenticated: state.status === SessionStatus.AUTHENTICATED    isLoading: state.status === SessionStatus.LOADING,
