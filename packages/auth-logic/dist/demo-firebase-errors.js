 ...
 */
import { FirebaseErrorHandler, useFirebaseErrorHandler, } from "./firebase-error-handler";
...
export async function exampleAuthOperation() {
    try {
        ...
        const userFriendlyError = FirebaseErrorHandler.handleError(_error);
...
export function ExampleAuthComponent() {
    ...
    const handleSignIn = async (_email) => {
        try {
            // Your Firebase Auth sign-in logic here
            // await signInWithEmailAndPassword(auth, email, password);
        }
        catch (_error) {
            const errorDisplay = getErrorDisplay(_error);
            // Show error to user
            alert(`${errorDisplay.message}\nAction: ${errorDisplay.actionButtonText}`);
...
export async function exampleFunctionCall() {
    try {
        ...
        const userFriendlyError = FirebaseErrorHandler.handleFunctionsError(_error);
      ...
      return (
        <FirebaseErrorBoundary
          onError={(error, errorInfo) => {
            ...
            logger.error('Error boundary caught', { error: _error });
          }}
        >
          <YourAppContent />
        </FirebaseErrorBoundary>
...
export const commonErrorScenarios = {
    // Auth errors
    "auth/user-not-found": () => {
        const _error = FirebaseErrorHandler.handleAuthError(new Error("auth/user-not-found"));
        // "No account found with this email address..."
        // "sign-up"
    },
    "auth/too-many-requests": () => {
        const _error = FirebaseErrorHandler.handleAuthError(new Error("auth/too-many-requests"));
    ...
    "functions/permission-denied": () => {
        const _error = FirebaseErrorHandler.handleFunctionsError(new Error("functions/permission-denied"));
    ...
    // Generic errors
    "generic-error": () => {
        const _error = FirebaseErrorHandler.handleError(new Error("Something went wrong"));
...
//# sourceMappingURL=demo-firebase-errors.js.map