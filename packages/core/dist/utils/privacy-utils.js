...
exports.privacyPresets = exports.privacyUtils = void 0;
exports.usePrivacyUtils = usePrivacyUtils;
exports.withPrivacyCheck = withPrivacyCheck;
...
class PrivacyUtils {
    ...
    async checkVisibility(targetUserId, context) {
        try {
    ...
    async batchCheckVisibility(userIds, context) {
        try {
    ...
    async toggleGhostMode(enabled, level, duration) {
        try {
    ...
    async getGhostModeStatus(userId) {
        try {
    ...
    filterUserData(userData, visibility) {
    ...
    shouldShareActivity(privacySettings, activityType) {
        if (!privacySettings)
            return true;
...
exports.privacyUtils = PrivacyUtils.getInstance();
// React hooks for easy integration
function usePrivacyUtils() {
    return exports.privacyUtils;
}
// Higher-order component for privacy-aware components
function withPrivacyCheck(Component) {
    return function PrivacyCheckedComponent(props) {
        const [visibility, setVisibility] = react_1.default.useState(null);
        const [loading, setLoading] = react_1.default.useState(true);
        react_1.default.useEffect(() => {
            if (props.userId) {
                exports.privacyUtils.checkVisibility(props.userId).then(result => {
        ...
        if (props.userId && visibility && !visibility.canSeeProfile) {
            return (0, jsx_runtime_1.jsx)("div", { children: "Profile not visible" });
        }
...
exports.default = PrivacyUtils;
//# sourceMappingURL=privacy-utils.js.map