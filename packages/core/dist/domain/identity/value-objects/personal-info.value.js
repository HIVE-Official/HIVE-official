...
class PersonalInfo extends ValueObject_base_1.ValueObject {
    constructor(props) {
        super(props);
    }
    static create(props) {
        ...
        if (bio && bio.length > this.MAX_BIO_LENGTH) {
            return Result_1.Result.fail(`Bio must be at most ${this.MAX_BIO_LENGTH} characters`);
        }
        if (graduationYear !== null) {
            if (graduationYear < this.MIN_GRAD_YEAR || graduationYear > this.MAX_GRAD_YEAR) {
                return Result_1.Result.fail('Invalid graduation year');
            }
        }
        return Result_1.Result.ok(new PersonalInfo({
