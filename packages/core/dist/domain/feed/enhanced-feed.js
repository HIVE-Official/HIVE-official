...
class EnhancedFeed extends AggregateRoot_base_1.AggregateRoot {
    ...
    static create(userId, campusId, id) {
        const feedIdResult = feed_id_value_1.FeedId.createForUser(userId.value, campusId.value);
        if (feedIdResult.isFailure) {
            return Result_1.Result.fail(feedIdResult.error || 'Failed to create feed ID');
        }
        const feedProps = {
        ...
        return Result_1.Result.ok(new EnhancedFeed(feedProps, id));
    }
    static createWithCampus(userId, campusId) {
        const campusIdResult = campus_id_value_1.CampusId.create(campusId);
        if (campusIdResult.isFailure) {
            return Result_1.Result.fail(campusIdResult.error || 'Invalid campus ID');
        }
    ...
    addItem(item) {
        ...
        if (this.props.items.some(i => i.itemId.value === item.itemId.value)) {
            return Result_1.Result.fail('Item already exists in feed');
        }
    ...
    removeItem(itemId) {
        ...
        if (index === -1) {
            return Result_1.Result.fail('Item not found in feed');
        }
...
exports.EnhancedFeed = EnhancedFeed;
