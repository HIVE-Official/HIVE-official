...
const event_batching_service_1 = require("../services/event-batching.service");
...
class AnalyticsSession {
    ...
    setupBatching(batchSize = 100, flushInterval = 30000, onFlush) {
        this.batchingService = new event_batching_service_1.EventBatchingService(batchSize, flushInterval, onFlush);
    }
    /**
    ...
    addEvent(event) {
        ...
        const processedEvent = privacy_service_1.PrivacyService.applyPrivacySettings(event, this.privacyPreferences);
        if (!processedEvent) {
    ...
    cleanupExpiredEvents(retentionDays = 90) {
        this.events = privacy_service_1.PrivacyService.removeExpiredEvents(this.events, retentionDays);
    }
    /**
     * Generate privacy report for session
     */
    generatePrivacyReport() {
        const baseReport = privacy_service_1.PrivacyService.generatePrivacyReport(this.events);
        return {
            sessionId: this.sessionId,
            ...baseReport,
        };
