...
export enum FeedContentType {
  ...
  TOOL = "tool", // Lower priority (Pulse, PromptPost, etc.)
}
...
export enum FeedContentSource {
  ...
  JOINED_SPACE = "joined_space", // +10 point boost
}
...
export interface RankableFeedContent {
  ...
  spaceId?: string; // Space the content came from
  authorId: string;
  createdAt: Date;
  engagementCount: number; // Total emoji reactions
  hasSurgeTag: boolean; // Tool surge state
...
export class MainFeedRanker {
  ...
  private static readonly SURGE_BOOST = 25; // Posts from surging Tools = +25pts
  private static readonly EVENT_DENSITY_CAP = 5; // Hard limit 5 events/day
  private static readonly ARCHIVE_THRESHOLD_HOURS = 72; // After 72h collapse into "Archive"
  ...
  private static calculateEngagementScore(engagementCount: number): number {
    if (engagementCount === 0) return 0;
    if (engagementCount <= 10) return engagementCount * 2; // Linear up to 20pts
  ...
  static rankContent(contents: RankableFeedContent[]): RankableFeedContent[] {
    ...
    const recentEvents = contents.filter(
      (content) =>
        content.type === FeedContentType.EVENT && content.createdAt > oneDayAgo
    );
    ...
    const cappedEventIds = new Set(cappedEvents.map((e: RankableFeedContent) => e.id));

    // Filter out excess events
    const filteredContents = contents.filter(
      (content) =>
        content.type !== FeedContentType.EVENT ||
    ...
    const scoredContents = filteredContents.map((content) => {
      ...
      const totalScore =
        recencyScore + engagementScore + typeScore + sourceBoost + surgeBoost;
  ...
  static shouldArchive(
    content: RankableFeedContent,
    minEngagementThreshold = 5
  ): boolean {
    const hoursOld =
      (new Date().getTime() - content.createdAt.getTime()) / (1000 * 60 * 60);

    return (
      hoursOld > this.ARCHIVE_THRESHOLD_HOURS &&
...
export interface ToolSurgeState {
  ...
  toolName: string;
  surgeStartedAt: Date;
  surgeScore: number; // Based on usage frequency
 ...
 */
export class ToolSurgeTracker {
  private static readonly SURGE_DURATION_HOURS = 24;
  private static readonly MIN_PARTICIPANTS_FOR_SURGE = 5;
  private static readonly SURGE_SCORE_THRESHOLD = 50; // Minimum score to trigger surge
  ...
  static isToolSurging(toolId: string, surgeStates: ToolSurgeState[]): boolean {
    const now = new Date();
    const toolState = surgeStates.find((state: ToolSurgeState) => state.toolId === toolId);

    if (!toolState || !toolState.isActive) return false;

    const hoursAgo =
      (now.getTime() - toolState.surgeStartedAt.getTime()) / (1000 * 60 * 60);
  ...
  static calculateSurgeScore(
    ...
    const usageDensity =
      recentUsageCount / (participantCount * timeWindowHours);

    // Participation factor: how many unique users are using the tool
    const participationFactor = Math.min(
   ...
   */
  static updateSurgeState(
    toolId: string,
    recentUsageCount: number,
    participantCount: number,
    existingStates: ToolSurgeState[]
  ): ToolSurgeState {
    const now = new Date();
    const surgeScore = this.calculateSurgeScore(
      recentUsageCount,
      participantCount,
      1
    ); // 1 hour window
    ...
    const existingState = existingStates.find(
      (state: ToolSurgeState) => state.toolId === toolId
    );
    ...
    return {
      ...
      participantCount,
      isActive:
        surgeScore >= this.SURGE_SCORE_THRESHOLD &&
        participantCount >= this.MIN_PARTICIPANTS_FOR_SURGE,
    };
  }
}
...
export const FeedContentSchema = z.object({
  id: z.string(),
  type: z.nativeEnum(FeedContentType),
  source: z.nativeEnum(FeedContentSource),
  spaceId: z.string().optional(),
  authorId: z.string(),
  createdAt: z.date(),
  engagementCount: z.number().min(0),
  hasSurgeTag: z.boolean(),
  metadata: z.record(z.unknown()),
});

export const ToolSurgeStateSchema = z.object({
  toolId: z.string(),
  toolName: z.string(),
  surgeStartedAt: z.date(),
  surgeScore: z.number(),
