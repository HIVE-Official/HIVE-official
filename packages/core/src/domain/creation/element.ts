...
export const ElementId = z
  ...
  .regex(
    /^[a-z][a-zA-Z0-9]*-v\d+$/,
    "Element ID must follow format: elementName-v1"
  );
...
export const ElementPresetSchema = z.object({
  ...
  config: z.unknown(), // Will be validated against specific element config schema
...
export const ElementSchema = z.object({
  ...
  defaultConfig: z.unknown(), // Default configuration object
...
export const ElementInstanceSchema = z.object({
  ...
  config: z.unknown(), // Configuration object (validated against element's schema)
  position: z.object({
    x: z.number(),
    y: z.number(),
...
// Utility functions
export const parseElementId = (
  elementId: ElementId
): { name: string; version: number } => {
  const match = elementId.match(/^([a-z][a-zA-Z0-9]*)-v(\d+)$/);
  if (!match) throw new Error(`Invalid element ID format: ${elementId}`);
...
export const getLatestElementVersion = (
  elements: Element[],
  baseName: string
): Element | null => {
...
export const validateElementConfig = (
  element: Element,
  config: unknown
): boolean => {
  try {
