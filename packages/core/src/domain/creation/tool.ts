import { z } from 'zod'
import { ElementInstanceSchema, type ElementInstance } from './element'
...
// Utility functions
export const createToolDefaults = (
  ownerId: string,
  data: CreateTool
...
export const canUserEditTool = (tool: Tool, userId: string): boolean => {
  if (tool.ownerId === userId) return true
  return tool.collaborators.some(collab => 
    collab.userId === userId && collab.permission === 'edit'
  )
}

export const canUserViewTool = (tool: Tool, userId: string): boolean => {
  if (tool.isPublic) return true
  if (tool.ownerId === userId) return true
  return tool.collaborators.some(collab => collab.userId === userId)
}

export const getNextVersion = (currentVersion: string, changeType: 'major' | 'minor' | 'patch'): string => {
...
export const determineChangeType = (
  oldElements: ElementInstance[],
  newElements: ElementInstance[]
): 'major' | 'minor' | 'patch' => {
...
export const validateToolStructure = (elements: ElementInstance[]): { isValid: boolean; errors: string[] } => {
  const errors: string[] = []
  ...
  // Check nesting depth
  const checkNestingDepth = (elementId: string, depth: number = 0): number => {
  ...
  const hasCircularReference = (elementId: string, visited: Set<string> = new Set()): boolean => {
