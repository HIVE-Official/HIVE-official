...
export function UniversalBottomNav({
  ...
  const handleNavigation = (href: string, isSpecial?: boolean) => {
  ...
  return (
    <div className={cn(
      ...
      "bg-hive-background-primary/95 backdrop-blur-lg",
      "border-t border-hive-border-subtle",
      "safe-area-pb",
      className
    )}>
      {/* Gradient overlay for blur effect */}
      <div className="absolute inset-0 bg-gradient-to-t from-hive-background-primary via-hive-background-primary/95 to-transparent pointer-events-none" />
        ...
        {primaryNavItems.map((item) => {
          ...
          return (
            <Button
              ...
              variant="ghost"
              size="sm"
              onClick={() => handleNavigation(item.href, item.isSpecial)}
              className={cn(
                "relative flex flex-col items-center gap-1 px-3 py-2 h-auto min-w-0 flex-1",
            ...
            >
              ...
              {active && !item.isSpecial && (
                <motion.div
                  layoutId="activeTab"
                  className="absolute -top-1 w-6 h-0.5 bg-[var(--hive-brand-secondary)] rounded-full"
                  initial={false}
              ...
              <div className="relative">
                ...
                {/* Badge */}
                {badgeCount > 0 && (
                  <Badge className="absolute -top-2 -right-2 h-4 w-4 p-0 flex items-center justify-center text-xs bg-red-500 text-[var(--hive-text-primary)] border-2 border-hive-background-primary">
