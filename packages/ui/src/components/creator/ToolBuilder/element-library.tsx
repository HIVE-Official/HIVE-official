...
import type { Element, ElementCategory } from "@hive/core";
...
const ElementIcon: React.FC<{ iconName: string; className?: string }> = ({
  iconName,
  className,
}) => {
  const iconMap = {
...
export const ElementLibrary: React.FC<ElementLibraryProps> = ({
  ...
  const { filteredElements, categorizedElements, popularElements } =
    useMemo(() => {
      ...
      // Categorize elements
      const categorized = {
        "Display & Layout": filtered.filter(
          (el) => el.category === "Display & Layout"
        ),
        "Inputs & Choices": filtered.filter(
          (el) => el.category === "Inputs & Choices"
        ),
        "Logic & Dynamics": filtered.filter(
          (el) => el.category === "Logic & Dynamics"
        ),
      };
    ...
    <div className={cn("flex flex-col h-full bg-card border-r", className)}>
      ...
      <div className="p-4 border-b">
        ...
        <Tabs
          ...
          onValueChange={(value) =>
            setSelectedCategory(value as ElementCategory | "all")
          }
        >
          <TabsList className="grid w-full grid-cols-2 h-auto">
            ...
            <TabsTrigger value="Display & Layout" className="text-xs">
              Display
            </TabsTrigger>
          </TabsList>
          <TabsList className="grid w-full grid-cols-2 h-auto mt-1">
            <TabsTrigger value="Inputs & Choices" className="text-xs">
              Inputs
            </TabsTrigger>
            <TabsTrigger value="Logic & Dynamics" className="text-xs">
              Logic
            </TabsTrigger>
          </TabsList>
        </Tabs>
