...
export const ToolBuilder: React.FC<ToolBuilderProps> = ({
  ...
  // History management
  const addToHistory = useCallback((newTool: Tool) => {
  ...
  const updateTool = useCallback((updater: (tool: Tool) => Tool) => {
  ...
  // Element management
  const addElement = useCallback((elementId: string, position: { x: number; y: number }) => {
  ...
  const updateElement = useCallback((elementId: string, updates: Partial<ElementInstance>) => {
    updateTool(tool => ({
      ...tool,
  ...
  const selectedElement = selectedElementId ? 
    currentTool.elements.find(el => el.id === selectedElementId) : null
      ...
      <div className={cn('flex flex-col h-screen bg-background', className)}>
        ...
        <div className="flex items-center justify-between px-4 py-2 border-t bg-muted/50 text-sm text-muted-foreground">
          ...
          <div className="flex items-center gap-4">
            ...
            <span>Last saved: {new Date(currentTool.updatedAt).toLocaleTimeString()}</span>
          </div>
        </div>
      </div>
    </DndProvider>
