...
import type { Transition, LayoutProps } from 'framer-motion';
...
export const layoutProps = {
  ...
  preserve: {
    layout: 'preserve-aspect' as const,
    transition: layoutTransitions.smooth,
  } as const,
} as const;
 ...
 * ```
 */
export function createSharedElement(id: string) {
  return {
    layoutId: id,
    transition: layoutTransitions.smooth,
  };
 ...
 * ```
 */
export function magicMoveProps(layoutId: string) {
  return {
    layoutId,
    transition: {
      type: 'spring' as const,
 ...
 */
export function findIndexByPosition<T>(
  array: T[],
