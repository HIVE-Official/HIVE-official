...
export const AlertIcons = {
  ...
  HIVE: () => (
    ...
    >
      ...
      <path d="M12 7c-2.76 0-5 2.24-5 5s2.24 5 5 5 5-2.24 5-5-2.24-5-5-5zm0 8c-1.654 0-3-1.346-3-3s1.346-3 3-3 3 1.346 3 3-1.346 3-3 3z"/>
    </svg>
  )
};
...
export const CampusAlerts = {
  ...
  AchievementAlert: ({ children, ...props }: Omit<AlertProps, 'variant' | 'icon'>) => (
    <Alert variant="success" icon={<AlertIcons.Success />} {...props}>
      {children}
    </Alert>
  ),
  
  // For course deadlines and important dates
  DeadlineAlert: ({ children, ...props }: Omit<AlertProps, 'variant' | 'icon'>) => (
    <Alert variant="warning" icon={<AlertIcons.Warning />} {...props}>
      {children}
    </Alert>
  ),
  
  // For system issues or errors
  SystemAlert: ({ children, ...props }: Omit<AlertProps, 'variant' | 'icon'>) => (
    <Alert variant="error" icon={<AlertIcons.Error />} {...props}>
      {children}
    </Alert>
  ),
  
  // For general information
  InfoAlert: ({ children, ...props }: Omit<AlertProps, 'variant' | 'icon'>) => (
    <Alert variant="info" icon={<AlertIcons.Info />} {...props}>
      {children}
    </Alert>
  ),
  
  // For HIVE-specific features and updates
  HIVEAlert: ({ children, ...props }: Omit<AlertProps, 'variant' | 'icon'>) => (
    <Alert variant="hive" icon={<AlertIcons.HIVE />} {...props}>
      {children}
    </Alert>
  )
};
