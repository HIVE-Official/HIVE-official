...
export function DataMappingRow({
  port,
  sourcePort: propSourcePort,
  targetPort: propTargetPort,
  sourceElementName,
  isSelected = false,
  removable = true,
  onClick,
  ...
  if (!sourcePort || !targetPort) {
    return (
      <div className={cn('data-mapping-row px-3 py-2 rounded-md border border-border', className)}>
    ...
    return (
      ...
      >
        ...
        <div className="flex-1 min-w-0">
          <p className="text-sm font-medium truncate">{port.name}</p>
          <DataTypeBadge type={sourceType as DataType} size="sm" variant="subtle" />
          {sourceElementName && (
