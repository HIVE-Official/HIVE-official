...
export interface PageContainerProps
  ...
  title?: string
  subtitle?: string
  breadcrumbs?: Array<{ label: string; icon?: React.ComponentType<any> }>
  actions?: React.ReactNode
}

const PageContainer = React.forwardRef<HTMLDivElement, PageContainerProps>(
  ({ className, variant, padding, maxWidth, title, subtitle, breadcrumbs, actions, children, ...props }, ref) => {
    return (
      <div
        ref={ref}
...
const PageFooter = React.forwardRef<HTMLDivElement, PageFooterProps>(
  ({ className, ...props }, ref) => {
    return (
      <div
        ref={ref}
        className={cn("mt-8 pt-6 border-t border-[var(--hive-border-default)]", className)}
        {...props}
      />
    )
  }
