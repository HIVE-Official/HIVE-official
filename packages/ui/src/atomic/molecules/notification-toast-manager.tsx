...
export const NotificationToastManager: React.FC<NotificationToastManagerProps> = ({
  notifications,
  previousCount = 0,
  onNavigate,
  shouldShowToast,
  ...
  const defaultShouldShowToast = useCallback((notification: HiveNotification): boolean => {
    // Only show toasts for high-priority, unread notifications
    if (notification.isRead) return false;

    // Behavioral criteria: "Someone needs you" gets immediate toast
    if (notification.category === 'someone_needs_you') return true;

    // High/urgent priority notifications
    if (notification.priority === 'urgent' || notification.priority === 'high') return true;

    // Direct connections (friend requests, mentions)
    if (notification.type === 'connection') return true;

    return false;
  }, []);

  // Create behavioral toast message
  const createToastFromNotification = useCallback((notification: HiveNotification): ToastNotificationData => {
    const baseToast: ToastNotificationData = {
...
export const createUrgentHelpToast = (
  senderName: string,
  helpType: string,
  actionUrl: string,
  onNavigate: (url: string) => void
...
export const createSocialProofToast = (
  achievement: string,
  percentile: string,
  actionUrl?: string,
  onNavigate?: (url: string) => void
...
export const createInsiderToast = (
  updateTitle: string,
