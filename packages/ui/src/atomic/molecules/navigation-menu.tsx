...
const NavigationBadge = React.forwardRef<HTMLSpanElement, NavigationBadgeProps>(
...
const NavigationItemComponent = React.forwardRef<HTMLButtonElement, NavigationItemComponentProps>(
  ({ item, isActive, size, variant, showIcon, showBadge, onClick }, ref) => {
...
export const NavigationMenu = React.forwardRef<HTMLElement, NavigationMenuProps>(
  ({
    ...
    const renderItems = (itemList: NavigationItem[]) => (
      itemList.map(item => (
    ...
    return (
      <nav
        ref={ref}
        className={cn(
          navigationContainerVariants({ orientation, layout }),
      ...
      >
        ...
        {groups.map(group => (
          <div key={group.id} className="contents">
            {/* Group Label (for sidebar layout) */}
            {group.label && layout === 'sidebar' && (
              <div className={cn(
                'px-4 py-2 mt-4 first:mt-0',
                'font-[var(--hive-font-family-primary)]',
                'font-semibold',
