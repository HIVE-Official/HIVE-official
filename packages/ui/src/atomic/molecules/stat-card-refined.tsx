...
const StatCard = React.forwardRef<HTMLDivElement, StatCardProps>(
  ({ 
    ...
    return (
      <Card 
        className={cn(
          statCardVariants({ semantic, className }),
        ...
        {...props}
      >
        <CardContent className="p-0 flex flex-col items-center gap-[var(--hive-space-3)]">
          {/* Icon container using composition rules */}
          <div className={cn(
            // Icon container from statCardComposition.icon
            "icon-container",
            "flex items-center justify-center",
            "p-[var(--hive-space-3)]",                    // 12px padding
            "rounded-[var(--hive-radius-lg)]",            // 12px border radius
          ...
          {/* Value using composition typography */}
          <div className="flex items-center gap-[var(--hive-space-1)]">
            <div 
              className={cn(
            ...
            {trend && trend.icon && (
              <div className={cn(
                "flex items-center",
                trend.direction === 'up' && "text-[var(--hive-success-primary)]",
                trend.direction === 'down' && "text-[var(--hive-error-primary)]",
                trend.direction === 'neutral' && "text-[var(--hive-text-muted)]"
              )}>
                <trend.icon className="w-[var(--hive-icon-small)] h-[var(--hive-icon-small)]" />
              </div>
            )}
          </div>
          
          {/* Label using composition typography */}
          <div 
            className={cn(
          ...
          {trend && trend.value && (
            <div className={cn(
              "text-[length:var(--hive-font-size-caption)]",    // 12px
              "font-[var(--hive-font-weight-medium)]",
              trend.direction === 'up' && "text-[var(--hive-success-primary)]",
              trend.direction === 'down' && "text-[var(--hive-error-primary)]",
              trend.direction === 'neutral' && "text-[var(--hive-text-muted)]"
            )}>
