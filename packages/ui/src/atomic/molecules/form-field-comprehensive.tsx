...
// === COMPONENT PROPS ===
export interface ComprehensiveFormFieldProps 
  ...
  // Campus context
  campusOptimized?: boolean; // Add campus-specific features
...
// === ICON MAPPING ===
const typeIconMap: Partial<Record<InputType, React.ComponentType<{ className?: string }>>> = {
  email: Mail,
  password: Lock,
  search: Search,
  text: User
...
// === MAIN COMPONENT ===
export const ComprehensiveFormField = React.forwardRef<HTMLInputElement, ComprehensiveFormFieldProps>(
  ({
    className,
    size,
    variant,
    ...
    const validationState: ValidationState = error ? 'error' : success ? 'success' : warning ? 'warning' : state;
    ...
    return (
      <div className={cn(formFieldVariants({ size, variant }), className)}>
        {/* Label */}
        {label && (
          <label
            htmlFor={id}
        ...
        <div className="relative">
          ...
          {IconComponent && (
            <div className="absolute left-3 top-1/2 -translate-y-1/2 pointer-events-none">
              <IconComponent className={cn(
                ...
                // State colors
                validationState === 'error' && 'text-[var(--hive-error-primary)]',
                validationState === 'success' && 'text-[var(--hive-success-primary)]',
                validationState === 'warning' && 'text-[var(--hive-warning-primary)]',
                // Focus color
          ...
          {showPasswordToggle && type === 'password' && (
            ...
            >
              {showPassword ? (
                <EyeOff className="hive-input-icon" />
              ) : (
                <Eye className="hive-input-icon" />
              )}
            </button>
          )}
        </div>
