...
// Mock calendar engine (Day/Week) with CSS grid; no external deps.
...
export const MockCalendarEngine: CalendarEngineComponent = ({
  view,
  events,
  timeStepMinutes = 30,
  startHour = 8,
  ...
  const handleCellMouseDown = (dayIndex: number, stepIndex: number) => {
    setSelecting({ startDay: dayIndex, startStep: stepIndex, endDay: dayIndex, endStep: stepIndex });
  };

  const handleCellMouseEnter = (dayIndex: number, stepIndex: number) => {
    if (!selecting) return;
    setSelecting({ ...selecting, endDay: dayIndex, endStep: stepIndex });
  };

  const handleMouseUp = () => {
    if (!selecting) return;
    finalizeSelection(selecting.startDay, selecting.startStep, selecting.endDay, selecting.endStep);
    setSelecting(null);
  };
  ...
  const renderEvent = (evt: CalendarEvent) => {
    ...
    return (
      <button
        ...
        className={cn(
          "absolute left-0 right-0 rounded-md px-2 py-1 text-left",
          "hover:bg-primary/25 focus:outline-none focus-visible:ring-2 focus-visible:ring-ring"
        )}
        style={{
          gridColumn: col + 2,
          gridRow: startStep + 2 + " / span " + durationSteps,
          translate: "0 0",
          backgroundColor: bg,
          border: `1px solid ${br}`,
      ...
      >
        ...
        {evt.location && (
          <div className="text-[11px] text-muted-foreground line-clamp-1">{evt.location}</div>
        )}
      </button>
    );
  };
  ...
  return (
    <div className={cn("w-full rounded-lg border border-border/60 bg-card overflow-hidden", className)} role="grid" aria-label={`${view} view calendar`} onMouseUp={handleMouseUp}>
      {/* Grid: first column time, subsequent columns days */}
      <div
        className="relative grid"
        style={{
          gridTemplateColumns: `80px repeat(${view === "day" ? 1 : 7}, 1fr)`,
          gridTemplateRows: `32px repeat(${totalSteps}, minmax(28px, 1fr))`,
        }}
      >
        {/* Header row */}
        ...
        {days.map((d) => {
          ...
          dayDate.setDate(baseWeekStart.getDate() + d);
          ...
          return (
            <div key={d} role="columnheader" className="sticky top-0 z-10 bg-card/95 backdrop-blur border-b border-border/50 px-2 py-1 text-caption font-semibold">
              {label}
            </div>
          );
        })}
        ...
        {Array.from({ length: totalSteps }).map((_, step) => (
          <React.Fragment key={step}>
            <div role="rowheader" className="border-r border-border/50 px-2 text-[11px] text-muted-foreground flex items-start">
              {step % (60 / timeStepMinutes) === 0 ? (
                <span>{String(startHour + Math.floor((step * timeStepMinutes) / 60)).padStart(2, "0")}:00</span>
              ) : (
            ...
            {days.map((d) => (
              <button
                key={`${d}-${step}`}
                type="button"
                onMouseDown={() => handleCellMouseDown(d, step)}
                onMouseEnter={() => handleCellMouseEnter(d, step)}
                className={cn(
                  "relative border-b border-l border-border/30 hover:bg-muted/30",
                  step % (60 / timeStepMinutes) === 0 && "border-border/50"
                )}
                role="gridcell"
                aria-label={`Select ${view === 'day' ? '' : ['Sun','Mon','Tue','Wed','Thu','Fri','Sat'][d] + ' '} ${String(startHour + Math.floor((step * timeStepMinutes) / 60)).padStart(2, "0")}:${String((step * timeStepMinutes) % 60).padStart(2, "0")}`}
              />
            ))}
          </React.Fragment>
        ))}
        ...
        <div className="pointer-events-none absolute inset-0 grid"
          style={{ gridTemplateColumns: `80px repeat(${view === "day" ? 1 : 7}, 1fr)`, gridTemplateRows: `32px repeat(${totalSteps}, minmax(28px, 1fr))` }}
        >
          <div />
          {days.map((d) => <div key={`h-${d}`} />)}
          {events.map((evt) => (
            <div key={evt.id} className="anim-fade-in">{renderEvent(evt)}</div>
          ))}
          {/* Selection highlight */}
          {selecting && (
            <div
              className="absolute rounded-md bg-primary/15 border border-primary/30 transition-all duration-200 ease-standard"
              style={{
                gridColumn: Math.min(selecting.startDay, selecting.endDay) + 2 + ' / span ' + (Math.abs(selecting.endDay - selecting.startDay) + 1),
                gridRow: Math.min(selecting.startStep, selecting.endStep) + 2 + ' / span ' + (Math.abs(selecting.endStep - selecting.startStep) + 1),
              }}
              aria-hidden
            />
          )}
