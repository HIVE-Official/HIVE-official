...
const SIZE_MAP: Record<string, number> = { xs: 16, sm: 24, default: 32, md: 36, lg: 48, xl: 64 };
...
export const HiveLogo = React.forwardRef<SVGSVGElement, HiveLogoProps>(
  (
    { className, variant = "gold", size = "default", showText = false, wrapperClassName, wordmarkClassName, label = "HIVE logo", ...props },
    ref
  ) => {
    ...
    const mark = (
      <svg
        ...
        width={resolvedSize}
        height={resolvedSize}
        className={cn("transition duration-smooth ease-standard", className)}
        {...props}
      >
        {variant === "gradient" && (
          <defs>
    ...
    return (
      <span className={cn("inline-flex items-center gap-2", wrapperClassName)}>
        {mark}
        <span className={cn("text-label font-label uppercase text-white", wordmarkClassName)}>HIVE</span>
      </span>
    );
  }
);
