...
export interface HiveUser {
  // Core properties (mapped from Firebase uid)
  id: string;
  email: string;
...
export function isHiveUser(user: any): user is HiveUser {
  return user && typeof user.id === 'string' && typeof user.email === 'string';
}

// Utility to map Firebase user to HiveUser
export function mapFirebaseUserToHiveUser(firebaseUser: any): HiveUser {
  return {
