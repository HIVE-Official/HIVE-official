...
// Tool API Types;
interface ToolResponse {tools: Tool[];
  pagination: {
    total: number;
    limit: number;
    offset: number;
...
interface CreateToolResponse {success: boolean;
  tool: Tool;
  message: string;}

interface UpdateToolResponse {success: boolean;
  tool: Tool;
  message: string;}

// Deployment API Types;
interface ToolDeploymentConfig {toolId: string;
...
interface ToolDeployment {id: string;
  toolId: string;
  deployedBy: string;
...
interface DeployedToolsResponse {deployments: ToolDeployment[];
  count: number;}

// Real-time API Types;
interface ToolUpdateEvent {id: string;
  toolId: string;
  toolName: string;
  deploymentId?: string;
  spaceId?: string;
...
interface ToolUpdatesResponse {success: boolean;
...
interface SpacesResponse {spaces: Space[];}

// Analytics Types;
interface ToolUsageData {toolId: string;
  spaceId: string;
  sessions: Array<{
    id: string;
    userId: string;
...
class ApiClient {
  ...
  async getSpaceTools(spaceId: string): Promise<DeployedToolsResponse> {
    return this.getDeployedTools({deployedTo: 'space',
      targetId: spaceId,)};
  }
