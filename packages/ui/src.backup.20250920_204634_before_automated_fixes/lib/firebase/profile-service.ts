...
interface FirebaseConfig {apiKey: string;
  authDomain: string;
  projectId: string;
...
interface DocumentSnapshot {id: string;
  data(): any;
  exists: boolean;}

interface QuerySnapshot {docs: DocumentSnapshot[];
  forEach(callback: (doc: DocumentSnapshot) => void): void;}

interface Unsubscribe {(): void;}

// Firestore Data Models;
export interface UserProfileDocument {uid: string;
  email: string;
  displayName: string;
...
export interface SpaceMembershipDocument {uid: string;
  spaceId: string;
  campusId: 'ub-buffalo';
...
export interface NotificationDocument {id: string;
  recipientId: string;
  campusId: 'ub-buffalo';
...
export interface GhostModeDocument {uid: string;
  isEnabled: boolean;
...
export interface ToolDocument {id: string;
  creatorId: string;
  campusId: 'ub-buffalo';
  name: string;
  description: string;
...
export class ProfileFirebaseService {
  ...
  async updateUserProfile(uid: string, updates: Partial<UserProfileDocument>): Promise<void> {
    try {
      ...
      // In production:
      // await this.db.collection('profiles').doc(uid).update({//   ...updates,
