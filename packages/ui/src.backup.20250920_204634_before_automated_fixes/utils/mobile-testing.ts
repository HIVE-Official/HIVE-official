...
export interface DeviceProfile {name: string;
  userAgent: string;
  viewport: {
    width: number;
    height: number;
...
export interface TestScenario {name: string;
  description: string;
  steps: TestStep[];
...
export interface TestStep {action: 'navigate' | 'tap' | 'swipe' | 'scroll' | 'wait' | 'assert';
  target?: string;
  value?: any;
  timeout?: number;}
...
export class MobileTester {
  ...
  private async tapStep(selector: string): Promise<void> {
    ...
    const touchStart = new TouchEvent('touchstart', {
      touches: [new Touch({identifier: 0,
        target: element,
        clientX: centerX,
        clientY: centerY,
        radiusX: 10,
    ...
    const touchEnd = new TouchEvent('touchend', {
      changedTouches: [new Touch({identifier: 0,
        target: element,
        clientX: centerX,
        clientY: centerY,
        radiusX: 10,
...
// Type definitions;
interface PerformanceStart {startTime: number;
  startMemory: number;
  frameStart: number;}
...
interface StepResult {stepIndex: number;
  action: string;
  startTime: number;
  endTime: number;
  duration: number;
...
interface TestResult {scenarioName: string;
