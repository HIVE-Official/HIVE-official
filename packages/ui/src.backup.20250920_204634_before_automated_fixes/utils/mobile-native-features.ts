...
export interface ShareData {title?: string;
  text?: string;
  url?: string;
  files?: File[];}
...
export async function shareContent(data: ShareData): Promise<boolean> {
  try {
    ...
    if (navigator.share) {
      await navigator.share({title: data.title,
        text: data.text,
        url: data.url;)};
      return true;
    }
...
export interface CameraOptions {quality?: number; // 0-1
  maxWidth?: number;
  maxHeight?: number;
...
export async function capturePhoto(options: CameraOptions = {}): Promise<File[]> {
  try {
    ...
    return selectFiles({accept: 'image/*',
      capture: 'environment',
...
export interface FileSelectOptions {accept?: string;
  multiple?: boolean;
...
export interface LocationData {latitude: number;
  longitude: number;
  accuracy: number;
  altitude?: number;
...
export interface LocationOptions {enableHighAccuracy?: boolean;
  timeout?: number;
  maximumAge?: number;}
...
// Device orientation;
export interface OrientationData {alpha: number; // Compass direction (0-360)
...
// Network information;
export interface NetworkInfo {type: 'wifi' | 'cellular' | 'bluetooth' | 'ethernet' | 'other' | 'unknown';
...
// Battery status;
export interface BatteryInfo {level: number; // 0-1
  charging: boolean;
...
export async function selectContacts(): Promise<ContactInfo[]> {try {
