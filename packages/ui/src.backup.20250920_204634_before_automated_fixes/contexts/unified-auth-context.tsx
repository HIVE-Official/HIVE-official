...
export interface HiveUser {id: string;
...
// Session Information;
export interface HiveSession {token: string | null;
  issuedAt?: string;
  expiresAt?: string;
  developmentMode?: boolean;
  lastActivity?: string;}
...
// Auth State;
export interface UnifiedAuthState {user: HiveUser | null;
...
export interface FirebaseAuthIntegration {listenToAuthChanges: (callback: (user: any) => void) => () => void;
  ...
  const initializeAuth = useCallback(async () => {
    try {
      ...
      if (firebaseIntegration && typeof window !== 'undefined') {
        ...
        if (firebaseIntegration.isEmailLinkSignIn()) {
          try {
            ...
            updateAuthState({error: error instanceof Error ? error.message : 'Email link sign-in failed',
              isLoading: false,)};
            return;
          }
        }
      ...
      if (persistedAuth) {
        ...
        if (isValid) {
          updateAuthState({user: persistedAuth.user,
      ...
      if (firebaseUser) {
        ...
        try {
          ...
          if (response.ok) {
            ...
            updateAuthState({user: null as any,
              session: null as any,
              isAuthenticated: false as any,
              isLoading: false,)};
          }
