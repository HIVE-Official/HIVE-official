...
interface RealtimeToolState {// Current tool state;
  toolState: any;
  isLoading: boolean;
  error: string | null;
...
interface RealtimeToolActions {// State management;
...
interface UseRealtimeToolOptions {toolId: string;
  deploymentId?: string;
  spaceId?: string;
  initialState?: any;
  autoConnect?: boolean;
...
export function useRealtimeTool(options: UseRealtimeToolOptions): [RealtimeToolState, RealtimeToolActions] {
  ...
  const fetchInitialState = useCallback(async () => {
    try {
      ...
      const response = await apiClient.getToolUpdates({toolId,
        deploymentId,
        spaceId,
  ...
  const updateToolState = useCallback(async (newState: any, optimistic: boolean = true) => {
    ...
    try {
      ...
      const response = await apiClient.syncToolState({toolId,
        deploymentId,
...
export function useToolState(toolId: string, initialState: any = {}) {
  const [realtimeState, realtimeActions] = useRealtimeTool({toolId,
    initialState,
    autoConnect: true,)};
