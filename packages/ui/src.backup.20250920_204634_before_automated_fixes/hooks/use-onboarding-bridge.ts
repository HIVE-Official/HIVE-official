...
export interface OnboardingData {fullName: string;
...
export interface OnboardingResult {success: boolean;
  user?: any;
...
export function useOnboardingBridge() {
  ...
  const getOnboardingProgress = useCallback(() => {
    ...
    if (user.email) completedSteps++; // Welcome (has account)
    if (user.fullName) completedSteps++; // Name step;
    if (user.major) completedSteps++; // Academics step;
    if (user.handle) completedSteps++; // Handle step;
    if (user.avatarUrl) completedSteps++; // Photo step (optional)
    if (user.onboardingCompleted) completedSteps = totalSteps; // All done;
    return {
      completedSteps,
      totalSteps,
  ...
  const createPostOnboardingSpaces = useCallback(async (
    onboardingData: OnboardingData;
  ) => {
    try {
      ...
      const cohortResponse = await fetch('/api/spaces/cohort/auto-create', {
        ...
        body: JSON.stringify({major: onboardingData.major,
      ...
      const autoJoinResponse = await fetch('/api/spaces/auto-join', {
        ...
        body: JSON.stringify({userId: unifiedAuth.user?.id;)},
      });
