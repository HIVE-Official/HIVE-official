...
interface UserState {profile: UserProfile | null;
...
interface SpacesState {spaces: Space[];
...
interface FeedState {posts: Post[];
...
interface UIState {activeTab: string;
  sidebarOpen: boolean;
...
interface OptimisticUpdate {id: string;
...
interface QueuedOperation {id: string;
  type: string;
  payload: any;
  timestamp: number;
  retryCount: number;
...
interface ConflictedItem {id: string;
  entity: string;
  localVersion: any;
  serverVersion: any;
...
interface ToastAction {label: string;
  action: () => void;
...
const RealTimeFeed = () => {
  ...
  const handleCreatePost = async () => {
    ...
    await createPostOptimistic({authorId: state.user.profile.id,
