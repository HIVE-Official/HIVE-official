...
interface RealTimeState {connectionStatus: 'disconnected' | 'connecting' | 'connected' | 'reconnecting' | 'error';
...
interface LiveUpdate {id: string;
...
interface CollaborativeDocument {id: string;
  title: string;
  content: string;
...
interface RealTimeNotification {id: string;
...
const RealTimeAnalytics = ({ connectionStatus }: { connectionStatus: string }) => {
  const [metrics, setMetrics] = useState({activeConnections: 0,
    messagesPerSecond: 0,
    averageLatency: 0,
    uptime: 0)};
