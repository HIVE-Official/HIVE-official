...
export interface InitializeRitualProps {currentStep?: number;
...
interface RitualStep {id: string;
  title: string;
  description: string;
...
const ProfileBasicsStep = ({ onComplete, initialData }: any) => {
  const [formData, setFormData] = useState({displayName: initialData?.displayName || '',
    bio: initialData?.bio || '',
