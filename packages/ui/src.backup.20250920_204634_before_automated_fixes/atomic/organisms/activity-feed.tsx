...
export interface ActivityFeedProps {feedType?: 'personal' | 'campus' | 'trending';
  userId?: string;
  spaceFilter?: string[];
  className?: string;}

interface FeedPost {id: string;
  type: 'space_event' | 'tool_success' | 'community_post' | 'ritual_milestone' | 'space_join' | 'collaboration';
  title: string;
  content: string;
  author: {
    id: string;
...
interface FeedFilters {postTypes: string[];
...
export function ActivityFeed({
  ...
  const [feedState, setFeedState] = useState({posts: [] as FeedPost[],
    isLoading: true,
    isLoadingMore: false,
    hasMore: true,
  ...
  useEffect(() => {
    const loadPosts = async () => {
      ...
      try {
        ...
        posts = posts.filter(post => 
          now.getTime() - new Date(post.createdAt).getTime() <= timeLimit;
        );
      ...
      <div className="flex items-center justify-between">
        <div>
          <h2 className="text-xl font-bold text-hive-text-primary">
            {feedType === 'personal' ? 'Your Activity Feed' : 
             feedType === 'campus' ? 'Campus Activity' : 
             'Trending Now'}
          </h2>
          <p className="text-sm text-hive-text-secondary">
            Powered by your summer ritual preferences;
          </p>
        </div>
          ...
          <div className="grid grid-cols-1 md:grid-cols-3 gap-4">
            <div>
              ...
              <div className="space-y-2">
                {['space_event', 'tool_success', 'ritual_milestone', 'collaboration'].map(type => (
      ...
      <div className="space-y-6">
          ...
          return (
            <Card key={post.id} className="p-6 hover:bg-hive-background-interactive transition-colors">
              ...
              <div className="mb-4">
                ...
                <p className="text-hive-text-secondary whitespace-pre-wrap">
                  {post.content}
                </p>
              </div>
                    ...
                    >
                      <div className="flex items-center space-x-3">
                        <div className={`w-8 h-8 rounded-lg flex items-center justify-center ${
                          attachment.type === 'tool' ? 'bg-hive-gold/20' :
                          attachment.type === 'event' ? 'bg-blue-500/20' : 'bg-hive-surface-elevated'
                        }`}>
                          {attachment.type === 'tool' && <Target className="h-4 w-4 text-hive-gold" />}
                          {attachment.type === 'event' && <Calendar className="h-4 w-4 text-blue-400" />}
                        </div>
                        <div>
        ...
        {feedState.hasMore && (
          <div className="text-center py-8">
            <div className="text-center py-8">
              <p className="text-hive-text-secondary mb-4">You're all caught up!</p>
              <Button;
                variant="secondary"
                size="sm"
                onClick={refreshFeed}
