...
export interface LockedFeedSkeletonProps {activationDate?: Date;
...
export function LockedFeedSkeleton({ 
  ...
  const [timeRemaining, setTimeRemaining] = useState({days: 0,
    hours: 0,
    minutes: 0,
    seconds: 0)};
  ...
  useEffect(() => {
    const timer = setInterval(() => {
      ...
      if (difference > 0) {
        setTimeRemaining({days: Math.floor(difference / (1000 * 60 * 60 * 24)),
