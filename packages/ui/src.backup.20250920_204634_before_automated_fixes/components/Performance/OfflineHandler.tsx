...
interface CampusOfflineContext {location: 'dorm' | 'library' | 'classroom' | 'outdoor' | 'commuting' | 'unknown';
...
interface OfflineActivity {actionsPerformed: {
    id: string;
...
interface OfflineHandlerProps {// Core configuration;
...
interface SyncResult {actionId: string;
  success: boolean;
  error?: string;
  serverResponse?: any;}

// Connection state with detailed information;
interface ConnectionState {isOnline: boolean;
    ...
    <div className={cn('relative', className)}>
      ...
      {showOfflineIndicator && !connectionState.isOnline && (
        <div className={cn(
          ...
          'bg-hive-gold text-black px-4 py-2 text-center text-sm font-medium',
      ...
      {connectionState.isReconnecting && (
        <div className={cn(
          'fixed top-0 left-0 right-0 z-50',
          'bg-hive-emerald text-white px-4 py-2 text-center text-sm font-medium'
        )}>
          <div className="flex items-center justify-center space-x-2">
      ...
      {process.env.NODE_ENV === 'development' && (
        <div className="fixed bottom-16 left-4 bg-black/90 text-white text-xs p-3 rounded-lg font-mono z-50 max-w-xs">
