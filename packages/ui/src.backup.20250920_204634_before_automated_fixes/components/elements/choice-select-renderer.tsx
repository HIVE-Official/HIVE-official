...
interface ChoiceOption {value: string;
  label: string;
  disabled?: boolean;}

interface ChoiceSelectConfig {// Element-specific properties (flexible)
  label: string;
...
interface ChoiceSelectRendererProps {element: ElementInstance;
...
export const ChoiceSelectRenderer: React.FC<ChoiceSelectRendererProps> = ({
  ...
  const behavior = useStandardElementBehavior({required: config.required,
