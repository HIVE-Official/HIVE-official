...
// Campus-specific network contexts;
type CampusNetworkContext = {type: 'campus-wifi' | 'dorm-ethernet' | 'library-wifi' | 'mobile-data' | 'public-wifi' | 'unknown';
  name?: string;
  knownIssues?: string[];
...
interface NetworkErrorProps {// Error details;
...
function useNetworkMonitoring() {
  ...
  const [networkCondition, setNetworkCondition] = useState<NetworkCondition>('unknown');
...
function detectCampusNetwork(): CampusNetworkContext {
...
function generateCampusMessage(
  errorType: NetworkErrorType,
  campusContext: CampusNetworkContext,
  networkCondition: NetworkCondition;
): {
...
export const NetworkError: React.FC<NetworkErrorProps> = ({
  ...
  originalError,
  networkCondition: propNetworkCondition,
  campusContext: propCampusContext,
  onRetry,
  onOfflineMode,
  enableOfflineMode = true,
  showCampusStatus = true,
  enableNetworkTips = true,
  ...
  return (
    <div className={cn(
      ...
      'bg-gradient-to-br from-hive-background-primary via-hive-background-secondary/20 to-hive-background-primary',
      className;
    )}>
      {/* Network status indicator */}
      <div className={cn(
        'w-24 h-24 rounded-full flex items-center justify-center mb-6',
        'bg-hive-background-secondary/30 border-2 border-dashed',
        getConditionColor(networkCondition).replace('text-', 'border-')
      )}>
        <span className="text-4xl" role="img" aria-label="Network status">
          {getNetworkIcon(networkCondition)}
        </span>
      </div>
      ...
      {showCampusStatus && (
        <div className="mt-6 p-4 bg-hive-background-secondary/20 rounded-lg border border-hive-border-default/20">
          <div className="flex items-center justify-center space-x-4 text-sm">
            <div className="flex items-center space-x-2">
              <span className={getConditionColor(networkCondition)}>
                {getNetworkIcon(networkCondition)}
              </span>
              <Text variant="body-sm" color="secondary">
                {networkCondition === 'offline' ? 'Offline' : `${networkCondition} connection`}
              </Text>
            </div>
            ...
            {networkMonitoring.downlink > 0 && (
              <div className="flex items-center space-x-2">
                <span className="text-hive-text-secondary">⚡</span>
                <Text variant="body-sm" color="secondary">
                  {networkMonitoring.downlink.toFixed(1)} Mbps;
                </Text>
              </div>
            )}
          </div>
      ...
      <div className="flex flex-col sm:flex-row gap-3 mt-8">
        ...
        {enableOfflineMode && onOfflineMode && (
          <Button;
            variant="secondary"
      ...
      {showAlternativeLocations && campusContext.type === 'campus-wifi' && (
        <div className="mt-6 p-4 bg-hive-gold/5 border border-hive-gold/20 rounded-lg">
          <Text variant="body-sm" color="secondary" className="font-medium mb-2">
            📍 Better signal locations:
          </Text>
          <div className="flex flex-wrap gap-2 justify-center">
            {['Library', 'Student Center', 'Academic Buildings', 'Outdoor Quads'].map((location) => (
              <span;
                key={location}
                className="px-3 py-1 bg-hive-gold/10 border border-hive-gold/30 rounded-full text-xs text-hive-text-secondary"
              >
                {location}
              </span>
            ))}
      ...
      {process.env.NODE_ENV === 'development' && (
        <details className="mt-8 text-left max-w-md">
          ...
          <div className="mt-2 p-3 bg-hive-background-secondary/30 rounded text-xs font-mono space-y-1">
            ...
            <div>Network Condition: {networkCondition}</div>
            <div>Connection Type: {networkMonitoring.connectionType}</div>
            <div>Downlink: {networkMonitoring.downlink} Mbps</div>
            <div>Online: {networkMonitoring.isOnline ? 'Yes' : 'No'}</div>
            <div>Campus Type: {campusContext.type}</div>
