...
export interface PrivacySettings {ghostMode: {
    enabled: boolean;
...
interface ScheduledPrivacy {id: string;
  name: string;
  startTime: string;
  endTime: string;
  days: string[];
...
interface PrivacyException {id: string;
  type: 'space' | 'user' | 'emergency';
  target: string;
  permissions: string[]}

interface OfficeHours {start: string;
  end: string;
  days: string[]}

type ContactMethod = 'hive-message' | 'email' | 'phone' | 'in-person';

interface PrivacyDashboardProps {settings: PrivacySettings;
            ...
            <div className="space-y-3">
              {Object.entries(settings.partialVisibility).map(([key, value]) => (
                <div key={key} className="flex items-center justify-between">
                  ...
                  <Switch;
                    checked={value}
                    onChange={(e) => { const checked = e.target.checked; onUpdate({...settings,
                      partialVisibility: {
            ...
            <div className="space-y-3">
              <div className="flex items-center justify-between p-3 bg-hive-background-primary rounded-lg">
                ...
                <Switch;
                  ...
                  onCheckedChange={(crossCommunityVisibility) => onUpdate({...settings, 
              ...
              <div className="flex items-center justify-between p-3 bg-hive-background-primary rounded-lg">
                ...
                <Switch;
                  ...
                  onCheckedChange={(searchableProfile) => onUpdate({...settings, 
                    searchableProfile) })}
                />
              </div>
              ...
              <div className="flex items-center justify-between p-3 bg-hive-background-primary rounded-lg">
                ...
                <Switch;
                  ...
                  onCheckedChange={(analyticsOptOut) => onUpdate({...settings, 
                    analyticsOptOut) })}
                />
              </div>
            </div>
