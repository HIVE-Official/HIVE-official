...
export interface ElementSchema {config: Record<string, ConfigProperty>;
  events: string[];
  props: string[];
  validation: Record<string, ValidationRule>;}

export interface ConfigProperty {type: 'string' | 'number' | 'boolean' | 'array' | 'object' | 'function';
  default: any;
  required?: boolean;
  options?: any[];
  min?: number;
...
export interface ElementDocumentation {description: string;
  examples: Array<{
    name: string;
...
export interface Size {width: number | 'auto' | string;
  height: number | 'auto' | string;}

export interface ElementStyle {backgroundColor?: string;
  borderColor?: string;
  borderWidth?: number;
  borderRadius?: number;
  padding?: Spacing;
...
export interface Spacing {top: number;
  right: number;
  bottom: number;
  left: number;}
...
export interface Collaborator {userId: string;
...
export interface ToolVersion {version: string;
  changelog: string;
  elements: ElementInstance[];
  publishedAt: Date;
  publishedBy: string;}

export interface ToolConfig {theme: 'auto' | 'light' | 'dark';
  primaryColor: string;
...
export interface ToolMetadata {templateId?: string;
...
export interface ToolBuilder {tool: Tool;
...
export interface Guide {id: string;
  type: 'horizontal' | 'vertical';
  position: number;
  color: string;}

export interface HistoryState {past: HistoryEntry[];
  present: HistoryEntry;
  future: HistoryEntry[];
  maxEntries: number;}

export interface HistoryEntry {timestamp: Date;
  action: string;
  elements: ElementInstance[];
...
export interface VisualBuilderProps {tool: Tool;
  elements: Element[];
...
export interface TemplateBuilderProps {tool: Tool;
...
export interface WizardBuilderProps {tool: Tool;
  onChange: (tool: Tool) => void;
  onSave: (tool: Tool) => void;
  onPreview: (tool: Tool) => void;
  isLoading?: boolean;}
...
export interface ToolTemplate {id: string;
  name: string;
  description: string;
  category: ToolCategory;
  difficulty: 'beginner' | 'intermediate' | 'advanced';
...
export interface EventHandler {id: string;
  event: string;
  action: EventAction;
  conditions?: Condition[];
  parameters?: Record<string, any>;}

export interface EventAction {type: 'navigate' | 'submit' | 'show' | 'hide' | 'update' | 'custom';
  target?: string;
  parameters: Record<string, any>;}

export interface Condition {id: string;
...
export interface DeploymentOptions {deployTo: 'personal' | 'space' | 'marketplace';
  targetId: string; // user ID, space ID, or 'marketplace'
  permissions: DeploymentPermissions;
  settings: DeploymentSettings;}

export interface DeploymentPermissions {canView: 'public' | 'authenticated' | 'members' | 'specific';
  canEdit: 'owner' | 'collaborators' | 'members';
  canFork: boolean;
  canComment: boolean;
  specificUsers?: string[];}

export interface DeploymentSettings {customDomain?: string;
...
export interface ElementPickerProps {elements: Element[];
  selectedCategory?: ElementCategory | 'all';
  onElementSelect: (element: Element) => void;
  searchQuery?: string;}

export interface ElementConfigProps {element: Element;
  instance: ElementInstance;
  onChange: (config: Record<string, any>) => void;}

export interface ToolPreviewProps {tool: Tool;
...
export interface BuilderContextType {state: ToolBuilder;
...
export interface CampusToolExtensions {spaceIntegration?: {
