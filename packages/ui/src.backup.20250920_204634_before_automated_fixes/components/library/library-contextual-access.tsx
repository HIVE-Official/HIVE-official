...
interface LibraryContextualAccessProps {space: any; // SpaceDefinition;
  onToolInstall?: (elementId: string, configuration?: any) => Promise<void>;
  onClose?: () => void;
  isOpen?: boolean;
  triggerRef?: React.RefObject<HTMLElement>}

interface ToolSuggestion {element: ElementDefinition;
  relevanceScore: number;
  reason: string;
  category: 'trending' | 'recommended' | 'space_specific' | 'popular'}

interface ContextualCategory {id: string;
  title: string;
  description: string;
...
const generateContextualSuggestions = (space: any): ToolSuggestion[] => {
  ...
  switch (space.category) {
    case 'study':
      suggestions.push({element: mockElements.find(e => e.id === 'study-timer')!,
        relevanceScore: 0.9,
    ...
    case 'social':
      suggestions.push({element: mockElements.find(e => e.id === 'poll-creator')!,
        relevanceScore: 0.8,
    ...
    case 'clubs':
      suggestions.push({element: mockElements.find(e => e.id === 'attendance-tracker')!,
        relevanceScore: 0.9,
  ...
  // Trending tools;
  suggestions.push({element: mockElements.find(e => e.id === 'mood-check')!,
    relevanceScore: 0.7,
