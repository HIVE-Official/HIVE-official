...
export interface BentoGridItem {id: string;
  title: string;
...
export interface BentoGridLayout {id: string;
  name: string;
  items: BentoGridItem[];
  columns: number;
  gap: number;
...
interface BentoGridProps {layout: BentoGridLayout;
    ...
    >
      ...
      {items.length === 0 && (
        ...
        >
          ...
          <p className="text-gray-600 mb-4">Add widgets to customize your dashboard</p>
          {editable && (
...
interface BentoGridItemComponentProps {item: BentoGridItem;
  isEditable: boolean;
  isDragging: boolean;
  isResizing: boolean;
...
export const createGridItem = (
  ...
  title: string,
  component: React.ReactNode,
  options: Partial<BentoGridItem> = {}}): BentoGridItem => ({
  id,
  title,
  component,
  size: 'medium',
