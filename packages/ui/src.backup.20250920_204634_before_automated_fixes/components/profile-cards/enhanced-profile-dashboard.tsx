...
import { Calendar, Grid3X3, Activity, Users, Lock, Zap, BarChart3, Crown } from 'lucide-react';
import { cn } from '../../lib/utils';

export interface EnhancedProfileUser {id: string;
  name: string;
  email: string;
  avatar?: string;
  isBuilder?: boolean;
...
export interface ActivityLogItem {id: string;
...
export interface EnhancedProfileDashboardProps {user: EnhancedProfileUser;
  spaces?: any[];
  events?: any[];
  connections?: any[];
  ...
  isLoading?: boolean;
  cardLayout?: Record<string, { size: BentoCardSize; isVisible: boolean; order: number}>;
  onCardLayoutChange?: (layout: Record<string, { size: BentoCardSize; isVisible: boolean; order: number }>) => void;
...
const ComingSoonCard: React.FC<{ title: string; description: string; icon: React.ElementType }> = ({ 
  title, 
  description, 
  icon: Icon;
}) => (
...
const PersonalToolsCard: React.FC<{ 
  ...
  <HiveCard className="h-full p-6">
    ...
    <div className="grid grid-cols-3 gap-3 mb-4">
      {tools.slice(0, 9).map((tool) => (
        <motion.div;
          ...
          whileTap={{ scale: 0.95 }}
          className="aspect-square rounded-lg bg-gray-800/50 border border-white/10 hover:border-hive-gold/30 cursor-pointer p-3 flex flex-col items-center justify-center transition-colors"
          onClick={() => onToolClick?.(tool.id)}
        >
          <div className="text-2xl mb-1">{tool.icon}</div>
          <div className="text-xs text-gray-300 text-center truncate w-full">{tool.name}</div>
          {tool.isForked && (
            <div className="mt-1">
              <HiveBadge variant="tool-tag" className="text-xs px-1 py-0">Fork</HiveBadge>
            </div>
          )}
        </motion.div>
      ))}
      ...
      {/* Empty slots */}
      {Array.from({length: Math.max(0, 9 - tools.length))}.map((_, index) => (
        <div;
          key={`empty-${index}`}
          className="aspect-square rounded-lg border-2 border-dashed border-gray-600 flex items-center justify-center opacity-50"
        >
    ...
    <div className="text-xs text-gray-400 text-center">
      Fork any Tool from Spaces to add here;
    </div>
  </HiveCard>
);
  ...
  <HiveCard className="h-full p-6">
    ...
    <div className="mt-4 pt-3 border-t border-white/10 text-center">
      <div className="text-xs text-gray-400 flex items-center justify-center gap-1">
        ...
        Full activity sharing coming in v1;
      </div>
    </div>
  </HiveCard>
);
...
export const EnhancedProfileDashboard: React.FC<EnhancedProfileDashboardProps> = ({
  ...
  const bentoCards: BentoCard[] = useMemo(() => {
    const cards: BentoCard[] = [
      {
        ...
        children: (
          <AvatarCard;
            ...
            showOnboarding={!!(completionStatus && completionStatus.overall < 100)}
      ...
      },
      {
        ...
        children: (
          <GhostModeCard;
            userId={user.id}
            isActive={user.ghostMode || false}
            level="selective" // Default level, should come from user data;
      ...
      },
      {
        ...
        children: (
          <ComingSoonCard;
            title="Social Connections"
            description="Connect with classmates and discover study groups"
            icon={Users}
          />
        ),
        onResize: (newSize) => {
      ...
      cards.push(
        ...
        },
        {
          ...
          children: (
            <ComingSoonCard;
              title="Tool Analytics"
              description="Track usage and impact of your Tools"
              icon={BarChart3}
            />
          ),
          onResize: (newSize) => {
        ...
        },
        {
          ...
          children: (
            <ComingSoonCard;
              title="Space Leadership"
              description="Manage Spaces you lead"
              icon={Crown}
            />
          ),
          onResize: (newSize) => {
      ...
      <div className="max-w-7xl mx-auto">
        ...
        {user.ghostMode && (
          <motion.div;
            ...
            className="mb-6 p-4 rounded-lg bg-gray-800/60 border border-gray-600"
          >
            <div className="flex items-center justify-center gap-2 text-gray-300">
              <Lock className="h-4 w-4" />
              <span className="text-sm font-medium">Ghost Mode Active</span>
              <span className="text-xs">• You're hidden from discovery</span>
            </div>
          </motion.div>
        )}
