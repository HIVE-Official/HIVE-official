...
const HiveButton = React.forwardRef(({ className, variant, size, asChild = false, loading = false, leftIcon, rightIcon, children, disabled, magneticHover = true, magneticIntensity = 'medium', rippleEffect = false, ...props }, ref) => {
    const isDisabled = disabled || loading;
    // Create enhanced spring transition based on variant
    const springTransition = liquidMetalUtils.createSpringTransition(variant === 'premium' || variant === 'glow' ? 'light' : 'standard', 'balanced', 'balanced');
    // Use Slot for asChild pattern (Radix compatibility)
    const Comp = asChild ? Slot : motion.button;
    const ButtonComponent = (_jsxs(Comp, { className: cn(hiveButtonVariants({ variant, size, className })), ref: ref, disabled: isDisabled, variants: magneticInteractions.hover, initial: "rest", whileHover: isDisabled ? "rest" : "magnetic", whileTap: isDisabled ? "rest" : "pressed", style: liquidMetalPerformance.gpuLayer, ...getMotionProps(props), children: [loading && (_jsx(Loader2, { className: "mr-2 h-4 w-4 animate-spin" })), leftIcon && !loading && (_jsx("span", { className: "mr-2 flex items-center", children: leftIcon })), children, rightIcon && (_jsx("span", { className: "ml-2 flex items-center", children: rightIcon }))] }));
    // Enhanced magnetic hover wrapper for premium interactions
    if (magneticHover && !isDisabled && (variant === 'premium' || variant === 'glow')) {
        return (_jsx(HiveMagneticHover, { intensity: magneticIntensity, disabled: isDisabled, children: ButtonComponent }));
    }
