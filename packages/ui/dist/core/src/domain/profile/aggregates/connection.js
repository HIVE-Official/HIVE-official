...
export class Connection extends AggregateRoot {
    ...
    static create(props, id) {
        ...
        const connectionIdResult = ConnectionId.createFromProfiles(profile1.value, profile2.value);
        if (connectionIdResult.isFailure) {
            return Result.fail(connectionIdResult.error);
        }
    ...
    block(blockedBy) {
        ...
        if (blockedBy.value !== this.props.profileId1.value &&
            ...
            return Result.fail('Blocker is not part of this connection');
        }
    ...
    unblock() {
        if (this.props.type !== ConnectionType.BLOCKED) {
            return Result.fail('Connection is not blocked');
        }
