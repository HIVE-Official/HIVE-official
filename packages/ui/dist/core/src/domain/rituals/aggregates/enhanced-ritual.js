...
export class EnhancedRitual extends AggregateRoot {
    get ritualId() {
    ...
    static create(props, id) {
        ...
        return Result.ok(new EnhancedRitual(ritualProps, id));
    }
    addParticipant(profileId) {
        ...
        if (this.props.participants.some(p => (typeof p === 'string' ? p : p.value) === id)) {
            return Result.fail('User is already participating');
        }
        ...
        if (!this.props.settings.allowLateJoin && this.props.startDate) {
            if (new Date() > this.props.startDate) {
                return Result.fail('Late join is not allowed for this ritual');
            }
        }
        // Add the participant
    ...
    removeParticipant(profileId) {
        ...
        if (index === -1) {
            return Result.fail('User is not participating');
        }
