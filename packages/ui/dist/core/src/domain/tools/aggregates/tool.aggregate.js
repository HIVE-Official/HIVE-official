...
export class Tool extends AggregateRoot {
    ...
    static create(props, id) {
        ...
        if (!props.name || props.name.trim().length === 0) {
            return Result.fail('Tool name is required');
        }
        if (!props.description || props.description.trim().length === 0) {
            return Result.fail('Tool description is required');
        }
        if (!props.elements || props.elements.length === 0) {
            return Result.fail('Tool must have at least one element');
        }
        ...
        // Fire domain event
        tool.addDomainEvent(new ToolCreatedEvent(tool.id, props.name, props.createdBy.value, props.spaceId?.value));
    ...
    publish() {
        if (this.props.status !== 'draft') {
            return Result.fail('Only draft tools can be published');
        }
        // Business Rule: Must have at least one element
        if (this.props.elements.length === 0) {
            return Result.fail('Cannot publish tool with no elements');
        }
        this.props.status = 'published';
        this.props.publishedAt = new Date();
        this.props.updatedAt = new Date();
        // Fire domain event
        this.addDomainEvent(new ToolPublishedEvent(this.id, this.props.name, this.props.visibility));
        return Result.ok();
    }
    /**
    ...
    archive() {
        if (this.props.status === 'archived') {
            return Result.fail('Tool is already archived');
        }
        this.props.status = 'archived';
        this.props.updatedAt = new Date();
        // Fire domain event
        this.addDomainEvent(new ToolArchivedEvent(this.id, this.props.name));
        return Result.ok();
    }
    /**
    ...
    fork(newName, newCreator, newSpaceId) {
        ...
        if (!this.props.permissions.canFork) {
            return Result.fail('This tool cannot be forked');
        }
        // Business Rule: Published tools are more trustworthy to fork
        if (this.props.status !== 'published') {
            return Result.fail('Only published tools can be forked');
        }
        // Create forked tool
        ...
        this.props.updatedAt = new Date();
        // Fire domain event
        this.addDomainEvent(new ToolForkedEvent(this.id, forkedTool.getValue().id, newCreator.value, this.props.analytics.forks));
        return forkedTool;
    }
    /**
    ...
    deployToSpaces(spaceIds) {
        ...
        if (this.props.status !== 'published') {
            return Result.fail('Only published tools can be deployed');
        }
        ...
        this.props.updatedAt = new Date();
        // Fire domain event
        this.addDomainEvent(new ToolDeployedEvent(this.id, this.props.name, spaceIds.map(s => s.value), this.props.deployedTo.length));
        return Result.ok();
    }
    /**
    ...
    undeployFromSpace(spaceId) {
        ...
        if (index === -1) {
            return Result.fail('Tool is not deployed to this space');
        }
    ...
    recordUse(userId) {
        ...
        if (this.props.status !== 'published') {
            return Result.fail('Only published tools can be used');
        }
        this.props.analytics.uses++;
        this.props.updatedAt = new Date();
        // Fire domain event
        this.addDomainEvent(new ToolUsedEvent(this.id, this.props.name, userId.value, this.props.analytics.uses));
        return Result.ok();
    }
    /**
    ...
    updateElements(elements) {
        if (this.props.status === 'published') {
            return Result.fail('Cannot modify elements of published tool. Create new version or unpublish first.');
        }
    ...
    grantEditAccess(profileId) {
        if (this.props.permissions.canEdit.some(p => p.value === profileId.value)) {
            return Result.fail('User already has edit access');
        }
    ...
    revokeEditAccess(profileId) {
        ...
        if (profileId.value === this.props.createdBy.value) {
            return Result.fail('Cannot revoke creator access');
        }
        const index = this.props.permissions.canEdit.findIndex(p => p.value === profileId.value);
        if (index === -1) {
            return Result.fail('User does not have edit access');
        }
