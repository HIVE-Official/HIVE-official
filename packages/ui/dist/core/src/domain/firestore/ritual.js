...
export const RITUAL_COLLECTIONS = {
    RITUALS: 'rituals',
    PARTICIPATION: (ritualId) => `rituals/${ritualId}/participants`,
    ANALYTICS: (ritualId) => `rituals/${ritualId}/analytics`,
...
export const RitualUtils = {
    ...
    canParticipate: (ritual, userId) => {
        if (!RitualUtils.isActive(ritual))
            return false;
        if (ritual.config.maxParticipants && ritual.totalParticipants >= ritual.config.maxParticipants)
            return false;
        // Add more participation rules as needed
        return true;
    },
    calculateProgress: (ritual) => {
        if (ritual.totalParticipants === 0)
            return 0;
        return (ritual.completedParticipants / ritual.totalParticipants) * 100;
    },
    getTimeRemaining: (ritual) => {
        const now = new Date();
