...
export class Space extends AggregateRoot {
    ...
    static create(props, id) {
        ...
        // Fire domain event
        space.addDomainEvent(new SpaceCreatedEvent(space.id, props.name.value, props.category.value, props.createdBy.value));
    ...
    addMember(profileId, role = 'member') {
        ...
        if (this.props.settings.maxMembers && this.memberCount >= this.props.settings.maxMembers) {
            return Result.fail('Space has reached maximum member capacity');
        }
    ...
    removeMember(profileId) {
        ...
        if (member.role === 'admin' && this.getAdminCount() === 1) {
            return Result.fail('Cannot remove the last admin');
        }
    ...
    updateMemberRole(profileId, newRole) {
        ...
        if (member.role === 'admin' && newRole !== 'admin' && this.getAdminCount() === 1) {
            return Result.fail('Cannot demote the last admin');
        }
    ...
    addTab(tab) {
        if (this.props.tabs.find(t => t.name === tab.name)) {
            return Result.fail('Tab with this name already exists');
        }
    ...
    incrementPostCount(postId, authorId) {
        ...
        if (postId && authorId) {
            this.addDomainEvent(new PostCreatedEvent(this.id, postId, authorId, this.props.postCount));
        }
    }
