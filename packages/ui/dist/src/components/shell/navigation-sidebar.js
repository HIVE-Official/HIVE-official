...
import { Home, Compass, Zap, User, Settings, BookOpen, Calendar, AlignJustify, Hash } from 'lucide-react';
...
function NavigationItem({ item, collapsed, isActive = false, router }) {
    ...
    return (_jsxs(Button, { variant: "ghost", onClick: handleClick, disabled: isNavigating, className: cn("w-full transition-all duration-300 ease-out group relative overflow-hidden rounded-lg", collapsed ? "justify-center h-12 p-0" : "justify-start h-12 px-4", "hover:scale-[1.02] hover:-translate-y-0.5", isNavigating && "pointer-events-none", isActive && "shadow-lg"), style: {
            background: isActive
