...
const SpaceCard = React.forwardRef(({ className, space, variant = "default", layout = "vertical", showActions = true, showStats = true, showCampusContext = false, showTags = true, maxTags = 3, onAction, onClick, onJoin, onLeave, ...props }, ref) => {
    ...
    const renderCampusContext = () => {
        ...
        return (_jsx("div", { className: "flex flex-wrap gap-1.5", children: displaySignals.map((signal, index) => (_jsxs(Badge, { variant: "secondary", className: "h-6 gap-1 px-2 text-xs bg-[#FFD700]/10 text-[#FFD700] border-[#FFD700]/20", children: [signal.icon, _jsx("span", { children: signal.label })] }, index))) }));
    };
    // Render helper: Tags
    const renderTags = () => {
        ...
        return (_jsxs("div", { className: "flex flex-wrap gap-1.5", children: [displayTags.map((tag, index) => (_jsxs(Badge, { variant: "secondary", className: "text-xs", children: ["#", tag] }, index))), remaining > 0 && (_jsxs(Badge, { variant: "outline", className: "text-xs", children: ["+", remaining] }))] }));
    };
    ...
    const renderActionButton = () => {
        ...
        if (isJoined) {
            return (_jsxs("div", { className: "flex items-center gap-2", children: [unreadCount > 0 && (_jsxs(Badge, { variant: "default", className: "h-5 px-2 text-xs bg-[#FFD700] text-black", children: [unreadCount, " new"] })), _jsxs(Button, { variant: "outline", size: "sm", className: "flex-1 text-sm font-medium", onClick: handleLeave, children: [_jsx(CheckCircle2, { className: "mr-2 h-4 w-4" }), "Joined"] })] }));
        }
