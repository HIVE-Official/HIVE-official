...
export class LiquidMetalOrchestrator {
    ...
    async triggerSpaceActivation(originId, connectedElements) {
        // Start origin ripple
    ...
    async triggerMilestone(type, elementIds) {
    ...
    async processCascadeQueue() {
        ...
        while (this.cascadeQueue.length > 0) {
            ...
            if (animation) {
                await animation();
                await new Promise(resolve => setTimeout(resolve, liquidTiming.cascade.stagger * 1000));
            }
        }
        this.isProcessingCascade = false;
    }
    // Queue cascade animation
    queueCascade(animation) {
...
export const liquidMetalUtils = {
    ...
    createEasedTransition(duration = 'smooth', easing = 'signature') {
        return {
...
export const liquidMetalPerformance = {
    ...
    respectsReducedMotion: (motionStyles) => ({
        ...motionStyles,
...
//# sourceMappingURL=hive-liquid-metal.js.map