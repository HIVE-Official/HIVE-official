...
[
    ...
    variants: {
        ...
        layout: {
            // Horizontal with labels (default)
...
const actionConfig = {
    like: {
        icon: Heart,
        defaultLabel: 'Like',
        activeColor: 'text-[var(--hive-error-primary)]', // Red heart for likes
        activeBg: 'bg-[var(--hive-error-background)]',
        activeBorder: 'border-[var(--hive-error-border)]',
        hoverColor: 'hover:text-[var(--hive-error-primary)]',
        hoverBg: 'hover:bg-[var(--hive-error-background)]/50',
        // Spring animation for like action
        animation: motionComposition.social.like
    },
    comment: {
        icon: MessageCircle,
        defaultLabel: 'Comment',
        activeColor: 'text-[var(--hive-info-primary)]', // Blue for comments
        activeBg: 'bg-[var(--hive-info-background)]',
        activeBorder: 'border-[var(--hive-info-border)]',
    ...
    share: {
        icon: Share2,
        defaultLabel: 'Share',
        activeColor: 'text-[var(--hive-success-primary)]', // Green for shares
        activeBg: 'bg-[var(--hive-success-background)]',
        activeBorder: 'border-[var(--hive-success-border)]',
...
const SocialActionButton = React.forwardRef(({ action, showLabel, showCount, disabled, onClick }, ref) => {
    ...
    return (_jsxs("button", { ref: ref, className: cn(
        ...
        'hive-social-btn', 'inline-flex items-center justify-center', 'relative', 'rounded-lg', 'font-[var(--hive-font-family-primary)]', 'font-medium', 'text-[var(--hive-text-secondary)]', 'bg-transparent', 
        // Border foundation
...
// === MAIN COMPONENT ===
export const SocialInteraction = React.forwardRef(({ className, size, layout, variant, actions, showLabels = true, showCounts = true, disabled = false, onAction, onLike, onComment, onShare, ...props }, ref) => {
    ...
    return (_jsx("div", { ref: ref, className: cn(socialInteractionVariants({ size, layout, variant }), className), role: "group", "aria-label": "Social actions", ...props, children: actions.map((action) => (_jsx(SocialActionButton, { action: action, showLabel: layout !== 'compact' && showLabels, showCount: showCounts, disabled: disabled, onClick: () => handleActionClick(action.type) }, action.type))) }));
});
...
//# sourceMappingURL=social-interaction.js.map