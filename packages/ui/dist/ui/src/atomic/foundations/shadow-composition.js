...
export const createShadowClass = (elevation) => {
...
export const createInteractiveShadowClass = (component, variant) => {
    const componentShadows = interactiveShadows[component];
    const variantShadows = componentShadows[variant];
    if (!variantShadows)
        return '';
    return Object.values(variantShadows).filter(v => typeof v === 'string').join(' ');
};
export const createCampusShadowClass = (category, variant) => {
    const categoryShadows = campusShadowPatterns[category];
    const variantShadow = categoryShadows[variant];
    if (!variantShadow)
        return '';
    return `${variantShadow.class} ${variantShadow.interaction}`;
};
//# sourceMappingURL=shadow-composition.js.map