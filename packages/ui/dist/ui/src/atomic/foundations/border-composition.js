...
export const createBorderClass = (width, color, radius) => {
    return `${borderWidths[width].className} ${color} ${borderRadius[radius].className}`;
};
export const createCampusBorderClass = (category, variant) => {
    const categoryBorders = campusBorderPatterns[category];
    const variantBorder = categoryBorders[variant];
    if (!variantBorder)
        return '';
    return `${variantBorder.border} ${variantBorder.radius} ${variantBorder.interactive}`;
};
export const createComponentBorderClass = (component, variant, state = 'default') => {
    const componentBorders = componentBorderPatterns[component];
...
//# sourceMappingURL=border-composition.js.map