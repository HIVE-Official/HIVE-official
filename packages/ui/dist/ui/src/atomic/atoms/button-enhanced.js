...
const Button = React.memo(React.forwardRef(({ className, variant, size, radius, loading = false, leftIcon, rightIcon, asChild = false, disabled, children, ...props }, ref) => {
    const Comp = asChild ? Slot : "button";
    // Memoize the className calculation to prevent unnecessary re-renders
    const buttonClassName = React.useMemo(() => cn(buttonVariants({ variant, size, radius, loading }), className), [variant, size, radius, loading, className]);
    // Memoize content to prevent unnecessary re-computation
    const content = React.useMemo(() => {
        if (loading) {
            return (_jsxs(_Fragment, { children: [_jsx(LoadingSpinner, { size: size }), children && _jsx("span", { className: "opacity-70", children: children })] }));
        }
        return (_jsxs(_Fragment, { children: [leftIcon && _jsx("span", { className: "flex-shrink-0", children: leftIcon }), children, rightIcon && _jsx("span", { className: "flex-shrink-0", children: rightIcon })] }));
    }, [loading, size, children, leftIcon, rightIcon]);
    return (_jsx(Comp, { className: buttonClassName, ref: ref, disabled: disabled || loading, ...props, children: content }));
}));
Button.displayName = "Button";
const LoadingSpinner = React.memo(({ size = "default" }) => {
    const spinnerSize = {
        xs: "h-3 w-3",
        sm: "h-3 w-3",
        default: "h-4 w-4",
