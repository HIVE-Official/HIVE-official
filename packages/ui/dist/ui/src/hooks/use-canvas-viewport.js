...
import { clampZoom, calculateZoomDelta, screenToCanvas, canvasToScreen, } from '../lib/hivelab-utils.js';
export function useCanvasViewport({ containerRef, minZoom = 0.1, maxZoom = 4, zoomSpeed = 0.001, }) {
    ...
    // Zoom Handlers
    // ============================================================================
    const zoomAt = useCallback((clientX, clientY, deltaZoom) => {
        ...
        const canvasPointBefore = screenToCanvas({ x: mouseX, y: mouseY }, state.viewport);
        // Apply new zoom
        const newViewport = { ...state.viewport, zoom: newZoom };
        // Calculate where the point is now with new zoom
        const canvasPointAfter = screenToCanvas({ x: mouseX, y: mouseY }, newViewport);
    ...
    return {
        ...
        resetZoom,
        zoomAt,
        screenToCanvas: useCallback((pos) => screenToCanvas(pos, state.viewport), [state.viewport]),
        canvasToScreen: useCallback((pos) => canvasToScreen(pos, state.viewport), [state.viewport]),
    };
}
//# sourceMappingURL=use-canvas-viewport.js.map