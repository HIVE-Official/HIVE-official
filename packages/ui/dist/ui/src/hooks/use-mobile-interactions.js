...
export function useHapticFeedback() {
    const triggerHaptic = useCallback((type = 'light') => {
        ...
        if ('hapticFeedback' in window && typeof window.hapticFeedback === 'function') {
            try {
...
// Swipe gesture hook
export function useSwipeGestures(config) {
    const { threshold = 50, restrained = false, velocity = 0.3, onSwipe, onSwipeLeft, onSwipeRight, onSwipeUp, onSwipeDown } = config;
    ...
    const handleTouchMove = useCallback((e) => {
        if (!startTouch.current || !isTracking)
            return;
        const touch = e.touches[0];
...
export function usePullToRefresh(config) {
    const { threshold = 80, onRefresh, resistance = 2.5, snapBackDuration = 300 } = config;
...
//# sourceMappingURL=use-mobile-interactions.js.map