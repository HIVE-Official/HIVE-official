...
export function useRealtimeTool(options) {
    ...
    const updateToolState = useCallback(async (newState, optimistic = true) => {
    ...
    const sendUpdate = useCallback(async (updateType, eventData) => {
        try {
    ...
    // Mark update as read
    const markUpdateAsRead = useCallback((updateId) => {
        setState(prev => ({
            ...prev,
            updates: prev.updates.map(update => update.id === updateId
                ? { ...update, isRead: true }
                : update),
        }));
    }, []);
...
export function useToolState(toolId, initialState = {}) {
...
//# sourceMappingURL=use-realtime-tool.js.map