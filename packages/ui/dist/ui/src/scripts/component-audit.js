...
class HiveComponentAuditor {
    ...
    async auditStoryCoverage(componentPath) {
        const issues = [];
    ...
    calculateScore(issues, compliance) {
        let score = 100;
    ...
    generateSummary() {
        ...
        const complianceStats = Object.keys(this.results[0]?.compliance || {}).reduce((acc, key) => {
            const compliantCount = this.results.filter(r => r.compliance[key]).length;
            acc[key] = total > 0 ? Math.round((compliantCount / total) * 100) : 0;
            return acc;
        }, {
            semanticTokens: 0,
            accessibility: 0,
...
//# sourceMappingURL=component-audit.js.map