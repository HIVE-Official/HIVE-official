...
export const ProfileLayout = React.forwardRef(({ variant = 'header', size = 'comfortable', children, className }, ref) => {
    const layout = layoutVariants[variant];
    const sizing = sizeVariants[size];
    return (_jsx("div", { ref: ref, className: cn(layout.container, sizing.spacing, className), children: children }));
});
ProfileLayout.displayName = 'ProfileLayout';
// === PROFILE CONTENT AREA ===
export const ProfileContent = React.forwardRef(({ children, className }, ref) => {
    return (_jsx("div", { ref: ref, className: cn('flex-1 min-w-0', // Prevent overflow
        className), children: children }));
});
ProfileContent.displayName = 'ProfileContent';
// === PROFILE ACTIONS AREA ===
export const ProfileActions = React.forwardRef(({ children, className }, ref) => {
    return (_jsx("div", { ref: ref, className: cn('flex-shrink-0', // Prevent shrinking
        className), children: children }));
});
ProfileActions.displayName = 'ProfileActions';
// === PROFILE GRID SYSTEM ===
export const ProfileGrid = React.forwardRef(({ columns = 2, gap = 'base', children, className }, ref) => {
    return (_jsx("div", { ref: ref, className: cn('grid', gridVariants.columns[columns], gridVariants.gaps[gap], className), children: children }));
});
...
export const ResponsiveProfileWrapper = React.forwardRef(({ children, className }, ref) => {
    return (_jsx("div", { ref: ref, className: cn(
        ...
        'xl:max-w-6xl', 
        // Padding
        'px-4 sm:px-6 lg:px-8', className), children: children }));
});
ResponsiveProfileWrapper.displayName = 'ResponsiveProfileWrapper';
// === UTILITY COMPONENTS FOR COMMON PATTERNS ===
// Avatar + Info layout
export const ProfileIdentityLayout = React.forwardRef(({ avatar, info, actions, layout = 'horizontal', className }, ref) => {
    return (_jsxs("div", { ref: ref, className: cn(layout === 'horizontal'
            ? 'flex items-start gap-4'
            : 'flex flex-col items-center text-center gap-3', className), children: [_jsx("div", { className: "flex-shrink-0", children: avatar }), _jsx("div", { className: "flex-1 min-w-0", children: info }), actions && (_jsx("div", { className: "flex-shrink-0", children: actions }))] }));
});
ProfileIdentityLayout.displayName = 'ProfileIdentityLayout';
// Stats grid layout
export const ProfileStatsLayout = React.forwardRef(({ children, columns = 4, className }, ref) => {
    return (_jsx("div", { ref: ref, className: cn('grid gap-4', columns === 2 ? 'grid-cols-2' : '', columns === 3 ? 'grid-cols-1 sm:grid-cols-3' : '', columns === 4 ? 'grid-cols-2 md:grid-cols-4' : '', className), children: children }));
});
ProfileStatsLayout.displayName = 'ProfileStatsLayout';
//# sourceMappingURL=profile-layout-system.js.map