...
export const PriorityCoordinationWidget = ({ priorities = [], isLoading = false, onActionTaken, onPriorityClick, onViewAll, ...baseProps }) => {
    ...
    const renderPriorityItem = (priority, index) => (_jsx(motion.div, { initial: { opacity: 0, y: 10 }, animate: { opacity: 1, y: 0 }, transition: { delay: index * 0.05 }, className: "group relative", children: _jsx("div", { className: cn('p-3 rounded-lg border transition-all duration-200 cursor-pointer', 'bg-hive-surface-elevated/50 border-hive-border-subtle/50', 'hover:bg-hive-surface-elevated border-hive-border-subtle', 'hover:shadow-lg hover:shadow-hive-gold/5'), onClick: () => onPriorityClick(priority.id), children: _jsxs("div", { className: "flex items-start gap-3", children: [_jsx("div", { className: cn('p-2 rounded-full shrink-0 mt-0.5', getSourceColor(priority.source)), children: getSourceIcon(priority.source) }), _jsxs("div", { className: "flex-1 min-w-0", children: [_jsxs("div", { className: "flex items-start justify-between gap-2 mb-2", children: [_jsx("h4", { className: "font-medium text-hive-text-primary line-clamp-1", children: priority.title }), priority.deadline && (_jsxs("div", { className: "flex items-center gap-1 text-xs text-hive-text-tertiary shrink-0", children: [_jsx(Clock, { className: "h-3 w-3" }), new Date(priority.deadline).toLocaleDateString()] }))] }), _jsx("p", { className: "text-sm text-hive-text-secondary line-clamp-2 mb-3", children: priority.description }), priority.context.participants && priority.context.participants.length > 0 && (_jsxs("div", { className: "flex items-center gap-2 mb-3", children: [_jsx(Users, { className: "h-3 w-3 text-hive-text-tertiary" }), _jsxs("span", { className: "text-xs text-hive-text-tertiary", children: [priority.context.participants.length, " people involved"] })] })), _jsxs("div", { className: "flex items-center gap-2 opacity-0 group-hover:opacity-100 transition-opacity", children: [priority.actions.slice(0, 2).map((action) => (_jsx(Button, { size: "sm", variant: action.type === 'primary' ? 'primary' : 'outline', onClick: (e) => {
                                            e.stopPropagation();
                                            onActionTaken(priority.id, action.id);
                                        }, className: "text-xs px-3 py-1", children: action.label }, action.id))), priority.actions.length > 2 && (_jsx(Button, { size: "sm", variant: "ghost", onClick: (e) => {
                                            e.stopPropagation();
