 ...
 */
"use client";
import { Home, Users, User, Wrench, Search, Bell, Settings, Heart, Bookmark, Calendar, GraduationCap, Building, Coffee, MapPin, Zap, Sparkles } from 'lucide-react';
export const hiveRoutes = {
    routes: {
        ...
        space: (spaceId) => `/s/${spaceId}`,
        user: (handle) => `/u/${handle}`,
        tool: (toolId) => `/t/${toolId}`,
        ritual: (ritualId) => `/r/${ritualId}`,
        // Deep navigation within entities
        spaceTools: (spaceId) => `/s/${spaceId}/tools`,
        spaceMembers: (spaceId) => `/s/${spaceId}/members`,
        spaceEvents: (spaceId) => `/s/${spaceId}/events`,
        userTools: (handle) => `/u/${handle}/tools`,
        // Utility routes
        search: '/search',
 ...
 */
export function createHiveNavigationSections(currentUser) {
    return [
 ...
 */
export function parseRouteEntity(path) {
    const patterns = [
        { regex: /^\/s\/([^\/]+)/, type: 'space' },
        { regex: /^\/u\/([^\/]+)/, type: 'user' },
        { regex: /^\/t\/([^\/]+)/, type: 'tool' },
 ...
 */
export function getRouteBreadcrumbs(path) {
...
//# sourceMappingURL=hive-navigation-routes.js.map