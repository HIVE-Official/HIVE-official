...
const ExecutionControls = ({ status, onExecute, onPause, onStop, onReset, permissions, isLoading = false }) => {
    ...
    return (_jsxs("div", { className: "flex items-center space-x-4", children: [_jsxs("div", { className: "flex items-center space-x-2", children: [getStatusIcon(), _jsx("span", { className: `text-sm font-medium ${getStatusColor()}`, children: getStatusText() })] }), _jsxs("div", { className: "flex items-center space-x-2", children: [status === 'idle' || status === 'completed' || status === 'error' || status === 'crashed' ? (_jsxs(Button, { onClick: handleExecute, disabled: !permissions.canExecute || isExecuting || isLoading, className: "bg-green-600 hover:bg-green-700 text-[var(--hive-text-inverse)] flex items-center space-x-2", children: [isExecuting || isLoading ? (_jsx(Loader2, { className: "w-4 h-4 animate-spin" })) : (_jsx(Play, { className: "w-4 h-4" })), _jsx("span", { children: "Run" })] })) : status === 'running' ? (_jsxs(Button, { onClick: onPause, disabled: !permissions.canExecute, className: "bg-yellow-600 hover:bg-yellow-700 text-[var(--hive-text-inverse)] flex items-center space-x-2", children: [_jsx(Pause, { className: "w-4 h-4" }), _jsx("span", { children: "Pause" })] })) : status === 'paused' ? (_jsxs(Button, { onClick: handleExecute, disabled: !permissions.canExecute, className: "bg-green-600 hover:bg-green-700 text-[var(--hive-text-inverse)] flex items-center space-x-2", children: [_jsx(Play, { className: "w-4 h-4" }), _jsx("span", { children: "Resume" })] })) : null, (status === 'running' || status === 'paused') && (_jsxs(Button, { onClick: onStop, disabled: !permissions.canExecute, variant: "secondary", className: "border-red-500 text-red-400 hover:bg-red-500/10 flex items-center space-x-2", children: [_jsx(Square, { className: "w-4 h-4" }), _jsx("span", { children: "Stop" })] })), _jsxs(Button, { onClick: onReset, disabled: !permissions.canEdit || status === 'running', variant: "secondary", className: "flex items-center space-x-2", children: [_jsx(RotateCcw, { className: "w-4 h-4" }), _jsx("span", { children: "Reset" })] })] })] }));
};
