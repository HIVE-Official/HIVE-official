...
export const HiveMembersSurface = React.forwardRef(({ className, mode, space, members: propMembers, currentUserId, isBuilder = false, canModerate = false, onViewProfile, onMessageMember, onInviteMember, onRemoveMember, onChangeRole, onBlockMember, viewMode = 'grid', showOfflineMembers = true, showMemberStats = true, maxMembers = 20, autoFetch = true, authToken, ...props }, ref) => {
    ...
    const normalizedMembers = members.map(member => ({
        ...
        role: (member.role === 'owner' || member.role === 'admin') ? 'builder' :
            (member.role in memberRoles ? member.role : 'member'),
    }));
