...
cn(componentBase.container, "relative overflow-hidden", touchTargets.comfortable, "focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-[var(--hive-brand-primary)]/30 focus-visible:ring-offset-2"), {
    variants: {
        variant: {
...
const HiveCard = React.forwardRef(({ className, variant, size, rounded, shadow, interactive = false, selected = false, goldAccent = false, magneticHover = true, magneticIntensity = 'subtle', animateEntrance = false, cascadeIndex, loading = false, children, onClick, 'aria-label': ariaLabel, 'aria-describedby': ariaDescribedby, 'data-testid': testId, ...props }, ref) => {
    ...
    const LoadingOverlay = loading ? (_jsx("div", { className: "absolute inset-0 bg-[var(--hive-background-primary)]/50 backdrop-blur-sm flex items-center justify-center z-10", children: _jsx("div", { className: "h-4 w-4 animate-spin border-2 border-[var(--hive-brand-primary)] border-t-transparent rounded-full" }) })) : null;
    const CardComponent = (_jsxs(motion.div, { className: cn(hiveCardVariants({
            variant: finalVariant,
            size,
            rounded,
            shadow,
            className
        }), loading && 'pointer-events-none', interactive && 'cursor-pointer'), ref: ref, onClick: loading ? undefined : onClick, style: liquidMetalPerformance.gpuLayer, tabIndex: interactive ? 0 : undefined, onKeyDown: interactive ? (e) => {
            if ((e.key === 'Enter' || e.key === ' ') && onClick && !loading) {
                e.preventDefault();
                onClick(e);
            }
        } : undefined, ...cardMotionProps, ...entranceProps, ...a11yProps, ...testingProps, ...getMotionProps(props), children: [LoadingOverlay, children] }));
