...
function CanvasComponent({ component, isSelected, onSelect, onUpdate, onDelete }) {
    ...
    const renderComponent = () => {
        switch (component.type) {
            ...
            case 'input':
                return _jsx(Input, { placeholder: component.placeholder || 'Enter text...', className: "w-full" });
            case 'textarea':
                return _jsx(Textarea, { placeholder: component.placeholder || 'Enter long text...', className: "w-full" });
            case 'button':
                return (_jsx(Button, { variant: component.style?.variant || 'default', size: component.style?.size || 'md', children: component.label || 'Button' }));
            case 'number':
                return _jsx(Input, { type: "number", placeholder: "0", className: "w-full" });
            case 'checkbox':
                return (_jsxs("label", { className: "flex items-center gap-2", children: [_jsx("input", { type: "checkbox", className: "rounded border-[var(--hive-border-default)]" }), _jsx("span", { children: component.label || 'Checkbox option' })] }));
            case 'select':
                return (_jsxs("select", { className: "w-full p-2 border border-[var(--hive-border-default)] rounded-md", children: [_jsx("option", { children: "Choose an option..." }), component.options?.map((option, i) => (_jsx("option", { value: option, children: option }, i)))] }));
            case 'divider':
                return _jsx("hr", { className: "border-[var(--hive-border-default)] w-full" });
            case 'card':
                return (_jsx(Card, { className: "p-4 min-h-[80px] border-dashed border-[var(--hive-border-default)]", children: _jsx("p", { className: "text-gray-500 text-center", children: "Card content goes here" }) }));
            default:
                return _jsx("div", { className: "p-2 bg-[var(--hive-background-secondary)] rounded text-sm", children: component.type });
        }
    };
    return (_jsxs("div", { ref: setNodeRef, style: style, onClick: onSelect, className: cn("relative group p-2 rounded-lg border-2 transition-all", isSelected ? "border-amber-400 bg-amber-50" : "border-transparent hover:border-gray-200", "cursor-pointer"), children: [_jsx("div", { ...listeners, ...attributes, className: cn("absolute -top-2 -left-2 w-6 h-6 bg-amber-500 rounded-full", "flex items-center justify-center cursor-grab active:cursor-grabbing", "opacity-0 group-hover:opacity-100 transition-opacity"), children: _jsx(Grip, { className: "w-3 h-3 text-[var(--hive-text-inverse)]" }) }), _jsx("button", { onClick: (e) => { e.stopPropagation(); onDelete(); }, className: cn("absolute -top-2 -right-2 w-6 h-6 bg-red-500 rounded-full", "flex items-center justify-center", "opacity-0 group-hover:opacity-100 transition-opacity"), children: _jsx(Trash2, { className: "w-3 h-3 text-[var(--hive-text-inverse)]" }) }), _jsx("div", { className: "pointer-events-none", children: renderComponent() })] }));
}
...
function PropertyPanel({ component, onUpdate }) {
    ...
    return (_jsxs("div", { className: "p-4 space-y-4", children: [_jsxs("div", { className: "flex items-center gap-2 mb-4", children: [_jsx(Settings, { className: "w-5 h-5 text-amber-600" }), _jsx("h3", { className: "font-semibold", children: "Component Settings" })] }), _jsxs("div", { children: [_jsx("label", { className: "block text-sm font-medium mb-1", children: component.type === 'text' || component.type === 'heading' ? 'Content' : 'Label' }), _jsx(Input, { value: component.label || '', onChange: (e) => onUpdate({ label: e.target.value }), placeholder: "Enter text..." })] }), (['input', 'textarea', 'number'].includes(component.type)) && (_jsxs("div", { children: [_jsx("label", { className: "block text-sm font-medium mb-1", children: "Placeholder" }), _jsx(Input, { value: component.placeholder || '', onChange: (e) => onUpdate({ placeholder: e.target.value }), placeholder: "Hint text..." })] })), component.type === 'select' && (_jsxs("div", { children: [_jsx("label", { className: "block text-sm font-medium mb-1", children: "Options (one per line)" }), _jsx(Textarea, { value: component.options?.join('\n') || '', onChange: (e) => onUpdate({ options: e.target.value.split('\n').filter(Boolean) }), placeholder: "Option 1\nOption 2\nOption 3", rows: 4 })] })), (['button', 'heading'].includes(component.type)) && (_jsxs("div", { children: [_jsx("label", { className: "block text-sm font-medium mb-1", children: "Size" }), _jsxs("select", { value: component.style?.size || 'md', onChange: (e) => onUpdate({
                            style: { ...component.style, size: e.target.value }
                        }), className: "w-full p-2 border border-[var(--hive-border-default)] rounded-md", children: [_jsx("option", { value: "sm", children: "Small" }), _jsx("option", { value: "md", children: "Medium" }), _jsx("option", { value: "lg", children: "Large" })] })] })), (['input', 'textarea', 'number', 'select'].includes(component.type)) && (_jsxs("label", { className: "flex items-center gap-2", children: [_jsx("input", { type: "checkbox", checked: component.required || false, onChange: (e) => onUpdate({ required: e.target.checked }), className: "rounded border-[var(--hive-border-default)]" }), _jsx("span", { className: "text-sm", children: "Required field" })] }))] }));
}
