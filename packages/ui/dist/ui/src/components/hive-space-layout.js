...
const hiveSpaceLayoutVariants = cva("relative w-full min-h-screen bg-[var(--hive-background-primary)]/5 backdrop-blur-sm", {
    variants: {
        mode: {
            view: "cursor-default",
...
export const HiveSpaceLayout = React.forwardRef(({ className, mode, layout, space, surfaces, isBuilder = false, canEdit = false, onSurfaceToggle, onSurfaceSettings, onBuilderMode, onSpaceSettings, onBackToDirectory, children, renderPinnedSurface, renderPostsSurface, renderEventsSurface, renderToolsSurface, renderChatSurface, renderMembersSurface, pinnedContent = [], posts = [], events = [], tools = [], messages = [], members = [], currentUserId, chatIsLocked = true, // Chat locked until v0.1.1
canCreateContent = true, canModerate = false, showBuilderHints = false, builderOnboarding, ...props }, ref) => {
    const [collapsedSurfaces, setCollapsedSurfaces] = useState(new Set());
    ...
    const activeSurfaces = useMemo(() => surfaces
        .map(surface => ({
        ...surface,
...
const SurfaceContainer = ({ surface, isCollapsed, onToggle, onSettings, canEdit, builderMode, index, compact = false, children }) => {
    const Icon = surface.config.icon;
    return (_jsxs(motion.section, { className: cn("relative bg-[var(--hive-background-primary)]/20 backdrop-blur-xl border border-white/10 rounded-2xl overflow-hidden", builderMode && "ring-2 ring-yellow-500/20 ring-offset-2 ring-offset-black/20", compact && "lg:max-h-96"), initial: { opacity: 0, y: 20 }, animate: { opacity: 1, y: 0 }, transition: { delay: index * 0.1 }, layout: true, children: [_jsxs("div", { className: "flex items-center justify-between p-4 border-b border-white/10", children: [_jsxs("div", { className: "flex items-center gap-3", children: [_jsx(Icon, { className: cn("w-5 h-5", surface.config.color) }), _jsxs("div", { children: [_jsx("h3", { className: "font-medium text-[var(--hive-text-primary)]", children: surface.customTitle || surface.config.label }), _jsx("p", { className: "text-xs text-gray-400", children: surface.config.description })] })] }), _jsxs("div", { className: "flex items-center gap-1", children: [surface.type === 'tools' && surface.toolCount && (_jsxs("span", { className: "px-2 py-1 bg-purple-500/20 text-purple-400 rounded-full text-xs mr-2", children: [surface.toolCount, " tools"] })), canEdit && (_jsx(motion.button, { className: "p-1.5 text-gray-400 hover:text-[var(--hive-text-primary)] rounded-lg hover:bg-[var(--hive-text-primary)]/5 transition-all duration-200", onClick: onSettings, whileHover: { scale: 1.05 }, whileTap: { scale: 0.95 }, children: _jsx(Settings, { className: "w-4 h-4" }) })), _jsx(motion.button, { className: "p-1.5 text-gray-400 hover:text-[var(--hive-text-primary)] rounded-lg hover:bg-[var(--hive-text-primary)]/5 transition-all duration-200", onClick: onToggle, whileHover: { scale: 1.05 }, whileTap: { scale: 0.95 }, children: isCollapsed ? _jsx(Maximize2, { className: "w-4 h-4" }) : _jsx(Minimize2, { className: "w-4 h-4" }) })] })] }), _jsx(AnimatePresence, { children: !isCollapsed && (_jsx(motion.div, { className: cn("p-4", compact && "max-h-80 overflow-y-auto"), initial: { opacity: 0, height: 0 }, animate: { opacity: 1, height: 'auto' }, exit: { opacity: 0, height: 0 }, transition: { duration: motionDurations.smooth }, children: children })) })] }));
};
// Individual surface components are imported from ./surfaces/
export { hiveSpaceLayoutVariants, surfaceConfig };
//# sourceMappingURL=hive-space-layout.js.map