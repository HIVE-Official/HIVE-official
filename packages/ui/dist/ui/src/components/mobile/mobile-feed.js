...
import { usePullToRefresh, useSwipeGestures, useHapticFeedback, useMobileViewport } from '../../hooks/use-mobile-interactions.js';
import { Card, CardContent, Button, Badge } from '../index.js';
...
export function MobileFeed({ posts, onRefresh, onLoadMore, onLike, onBookmark, onShare, onComment, hasMore = false, isLoading = false }) {
    ...
    const PullToRefreshIndicator = () => (_jsx("div", { className: "flex items-center justify-center py-4 transition-opacity duration-200", style: {
        ...
        }, children: isRefreshing ? (_jsxs(_Fragment, { children: [_jsx(Loader2, { className: "h-5 w-5 animate-spin text-blue-500 mr-2" }), _jsx("span", { className: "text-sm text-[var(--hive-text-muted)]", children: "Refreshing feed..." })] })) : canRefresh ? (_jsxs(_Fragment, { children: [_jsx(RefreshCw, { className: "h-5 w-5 text-blue-500 mr-2" }), _jsx("span", { className: "text-sm text-[var(--hive-text-muted)]", children: "Release to refresh" })] })) : (_jsxs(_Fragment, { children: [_jsx(ChevronDown, { className: "h-5 w-5 text-gray-400 mr-2 transition-transform duration-200", style: { transform: `rotate(${Math.min(pullDistance * 2, 180)}deg)` } }), _jsx("span", { className: "text-sm text-gray-400", children: "Pull to refresh" })] })) }));
