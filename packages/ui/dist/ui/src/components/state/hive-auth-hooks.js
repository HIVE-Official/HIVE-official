...
export function useBuilderProgression() {
    ...
    const advanceBuilderLevel = (newLevel) => {
        if (!currentUser)
            return;
        dispatch({
            type: 'AUTH_UPDATE_PROFILE',
...
export function usePrivacy() {
    ...
    const updatePrivacySettings = (updates) => {
        if (!currentUser)
            return;
        dispatch({
            type: 'AUTH_UPDATE_PROFILE',
    ...
    return {
        ...
        // Quick setters
        setProfileVisibility: (visibility) => updatePrivacySettings({ profileVisibility: visibility }),
        setToolSharingDefault: (sharing) => updatePrivacySettings({ toolSharingDefault: sharing }),
        toggleActivityTracking: () => updatePrivacySettings({ activityTracking: !privacy.activityTracking }),
        toggleFriendDiscovery: () => updatePrivacySettings({ friendDiscovery: !privacy.friendDiscovery })
    };
}
//# sourceMappingURL=hive-auth-hooks.js.map