...
shouldLoadResource(resource, PreloadResource, campusContext ?  : CampusPreloadContext, userContext ?  : UserPreloadContext);
boolean;
{
...
async;
loadResource(resource, PreloadResource, campusContext ?  : CampusPreloadContext);
Promise < any > {
...
selectLoadingStrategy(resource, PreloadResource, campusContext ?  : CampusPreloadContext);
keyof;
PreloadResource['strategy'];
{
...
async;
executeLoadingStrategy(resource, PreloadResource, strategy, keyof, PreloadResource['strategy'], campusContext ?  : CampusPreloadContext);
Promise < any > {
    switch(strategy) {
    },
    case: 'preload',
...
async;
prefetchResource(resource, PreloadResource);
Promise < any > {
    if(resource) { }, : .url
};
{
...
return resource.data;
async;
backgroundFetchResource(resource, PreloadResource);
Promise < any > {
...
return resource.data;
getResourceAs(type, ResourceType);
string;
{
    const typeMap = {
        'critical-css': 'style',
...
async;
processResponse(response, Response, resource, PreloadResource);
Promise < any > {
...
updateBandwidthEstimate(sizeKB, number, loadTimeMs, number);
{
