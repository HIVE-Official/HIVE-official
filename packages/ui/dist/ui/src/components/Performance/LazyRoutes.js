...
function useNavigationAnalytics(routes) {
    ...
    const currentRouteStartTime = useRef(Date.now());
    // Track route changes
    useEffect(() => {
        const handleRouteChange = () => {
            const currentPath = window.location.pathname;
            const currentRoute = routes.find(route => currentPath === route.path || currentPath.startsWith(route.path));
            if (currentRoute) {
                const now = Date.now();
...
export const LazyRoutes = ({ routes, campusContext, enablePredictiveLoading = true, enableHoverPreloading = true, enableAnalytics = true, loadingComponent, errorBoundary, maxConcurrentLoads = 3, preloadBudget = 1024, className, children }) => {
