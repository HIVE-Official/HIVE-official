...
const RippleEffect = ({ x, y, show }) => (_jsx(AnimatePresence, { children: show && (_jsx(motion.div, { className: "absolute pointer-events-none", style: { left: x, top: y }, initial: { scale: 0, opacity: 0.6 }, animate: { scale: 3, opacity: 0 }, exit: { opacity: 0 }, transition: { duration: 0.5, ease: "easeOut" }, children: _jsx("div", { className: "w-3 h-3 -ml-1.5 -mt-1.5 bg-gradient-radial from-[var(--hive-text-primary)]/30 to-transparent rounded-full" }) })) }));
const HivePremiumButton = React.forwardRef(({ className, variant, size, radius, magneticHover = true, rippleEffect = true, loading = false, leftIcon, rightIcon, children, onClick, disabled, ...props }, ref) => {
...
const HivePremiumButtonGroup = React.forwardRef(({ className, cascade = false, spacing = "md", children, ...props }, ref) => {
    ...
    return (_jsx("div", { ref: ref, className: cn("flex items-center", spacingClasses[spacing], className), ...props, children: cascade ? (React.Children.map(children, (child, index) => (_jsx(motion.div, { initial: { opacity: 0, y: 10 }, animate: { opacity: 1, y: 0 }, transition: {
                delay: index * 0.05,
                type: "spring",
                stiffness: 400,
                damping: 25
...
//# sourceMappingURL=hive-button-premium.js.map