...
const RefinedButton = React.forwardRef(({ className, variant, size, asChild = false, loading, children, ...props }, ref) => {
    const Comp = asChild ? Slot : "button";
    return (_jsx(Comp, { className: cn(refinedButtonVariants({ variant, size }), className), ref: ref, disabled: loading, ...props, children: loading ? (_jsxs("div", { className: "flex items-center gap-2", children: [_jsx(Loader2, { className: "h-4 w-4 animate-spin" }), _jsx("span", { className: "opacity-80", children: "Loading..." })] })) : (children) }));
});
RefinedButton.displayName = "RefinedButton";
// REFINED CARD: Apple-inspired elevation with tech precision
const refinedCardVariants = cva("relative bg-black border transition-all duration-200 ease-out", {
    variants: {
        variant: {
...
const RefinedCard = React.forwardRef(({ className, variant, padding, ...props }, ref) => {
    return (_jsx("div", { ref: ref, className: cn(refinedCardVariants({ variant, padding }), className), ...props }));
});
RefinedCard.displayName = "RefinedCard";
// REFINED TYPOGRAPHY: Technical precision with personality
const refinedTypographyVariants = cva("transition-all duration-200 ease-out", {
    variants: {
        variant: {
...
const RefinedTypography = React.forwardRef(({ className, variant, align, as: Component = "p", children, ...props }, ref) => {
...
const RefinedInput = React.forwardRef(({ className, variant, size, type, ...props }, ref) => {
    return (_jsx("input", { type: type, className: cn(refinedInputVariants({ variant, size }), className), ref: ref, ...props }));
});
...
function RefinedBadge({ className, variant, ...props }) {
    return (_jsx("div", { className: cn(refinedBadgeVariants({ variant }), className), ...props }));
}
...
//# sourceMappingURL=refined-components.js.map