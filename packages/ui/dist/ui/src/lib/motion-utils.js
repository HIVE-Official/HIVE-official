...
export function separateMotionProps(props) {
    const { layout, onDrag, onDragStart, onDragEnd, onAnimationStart, onAnimationEnd, onCopy, onCut, onPaste, size, breakpoint, theme, logoLayout, animated, 
...
export function getLogoMotionProps(props) {
    const { layout, onDrag, onDragStart, onDragEnd, onAnimationStart, onAnimationEnd, size, breakpoint, theme, showWordmark, adaptToContext, userPreferences, ...cleanProps } = props;
    return cleanProps;
}
...
export const hiveMotionVariants = {
    // Fade transitions
    fadeIn: {
        initial: { opacity: 0 },
        animate: {
...
export function getHiveMotionProps(type = 'button', enableMagnetic = true) {
    const base = {
        style: {
            willChange: 'transform',
            transformOrigin: 'center',
...
export function getCascadeProps(index, delay = 0.05) {
    return {
...
export function getStaggerProps(count, pattern = 'wave') {
    const delays = {
        wave: 0.03,
        ripple: 0.08,
        sequence: 0.12,
