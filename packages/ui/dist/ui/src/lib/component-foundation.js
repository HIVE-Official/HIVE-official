...
export const componentBase = {
    ...
    interactive: cn('inline-flex items-center justify-center', 'transition-all duration-300 ease-out', 'select-none backdrop-blur-sm', focusStyles({ type: 'default' }), responsiveAnimations.motion, 'disabled:pointer-events-none disabled:opacity-50'),
    // Container elements (cards, panels, etc.)
    container: cn('relative overflow-hidden', 'backdrop-blur-sm', responsiveAnimations.motion),
    // Input elements
    input: cn('w-full transition-all duration-200', touchTargets.comfortable, focusStyles({ type: 'default' }), 'disabled:cursor-not-allowed disabled:opacity-50'),
    // Text elements
    text: cn('text-[var(--hive-text-primary)]', responsiveAnimations.motion),
};
 ...
 */
export function createStandardComponent(displayName, baseClassName, variants, defaultVariants) {
 ...
 */
export function getStandardMotionProps(disabled, variant) {
 ...
 */
export function createLoadingIndicator(loading, size = 'md') {
    if (!loading)
        return null;
    const sizeClasses = {
        sm: 'h-3 w-3',
    ...
    return React.createElement('div', {
        className: cn('animate-spin border-2 border-current border-t-transparent rounded-full', sizeClasses[size], 'mr-2'),
        'aria-label': 'Loading...',
    });
}
/**
 ...
 */
export function createIconSlot(icon, position = 'left', loading) {
    if (!icon || loading)
        return null;
 ...
 */
export function getValidationProps(error, success) {
    if (error) {
        return {
            'aria-invalid': true,
