 ...
 * ```
 */
export async function expectNoA11yViolations(container, config = defaultAxeConfig) {
...
export function TestWrapper({ children }) {
    return (_jsxs("div", { className: "min-h-screen bg-[#0A0A0B] text-[#E5E5E7] antialiased font-sans", children: [_jsx("style", { children: `
 ...
 * ```
 */
export async function testKeyboardNavigation(element, options) {
 ...
 * ```
 */
export function expectAriaAttributes(element, attributes) {
 ...
 * ```
 */
export async function testFocusIndicator(element) {
    const { userEvent } = await import('@testing-library/user-event');
    const user = userEvent.setup();
    // Focus element
    await user.tab();
 ...
 * ```
 */
export function expectScreenReaderAnnouncement(element, options) {
    if (options.text) {
 ...
 * ```
 */
export function checkColorContrast(foreground, background, minimumRatio = 4.5) {
 ...
 * ```
 */
export async function testComponentAccessibility(component, options) {
