...
// ============================================================================
const NavigationLayout = memo(({ children, navigationState, navigationLayout }) => {
...
export const NavigationContainer = memo(({ user, onOpenCommandPalette, onOpenNotifications, unreadNotificationCount = 0, children, className, testId = 'navigation-container' }) => {
    ...
    const renderNavigation = () => {
        switch (state.mode) {
            case 'mobile-tabs':
                return (_jsx(MobileNavigation, { items: items, user: user, onNavigate: handleNavigate, testId: "mobile-navigation" }));
            ...
            default:
                ...
                return (_jsx(MobileNavigation, { items: items, user: user, onNavigate: handleNavigate, testId: "fallback-navigation" }));
        }
    };
    // ============================================================================
    // RENDER
    // ============================================================================
    return (_jsx(NavigationLayout, { navigationState: state, navigationLayout: layout, children: _jsxs("div", { className: cn('relative', className), "data-testid": testId, "data-navigation-mode": state.mode, "data-screen-size": state.screenSize, children: [_jsx(AnimatePresence, { mode: "wait", children: renderNavigation() }), children] }) }));
});
...
export const withNavigation = (Component) => {
    const WithNavigationComponent = memo((props) => {
        ...
        return (_jsxs(NavigationLayout, { navigationState: navigationState.state, navigationLayout: navigationState.layout, children: [_jsx(NavigationContainer, { user: user, onOpenCommandPalette: onOpenCommandPalette, onOpenNotifications: onOpenNotifications, unreadNotificationCount: unreadNotificationCount }), _jsx(Component, { ...componentProps, children: children })] }));
    });
