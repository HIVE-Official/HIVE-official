...
const generateStoryTemplate = (componentName: string, category: string, importPath: string) => {
    ...
    try {
      ...
      for (const file of files) {
        if (file.endsWith('.tsx') && !file.endsWith('.test.tsx') && !file.endsWith('.stories.tsx')) {
          ...
          const storyPath = path.resolve(__dirname, STORY_PATHS[category as keyof typeof STORY_PATHS], `${file.replace('.tsx', '.stories.tsx')}`);
          let hasStory = false;
          try {
          ...
          components.push({
            name: pascalCaseName,
            path: file,
            category: category as 'atoms' | 'molecules' | 'organisms' | 'templates',
            hasStory,
          });
        }
      }
...
async function generateMissingStories(components: ComponentInfo[]): Promise<void> {
