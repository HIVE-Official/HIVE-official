...
export const FileMetadataSchema = z.object({
    // Basic properties
    filename: z.string().min(1).max(255), originalName: z.string().min(1).max(255),
...
export const UploadConfigSchema = z.object({
    ...
    // Image-specific restrictions maxImageDimensions: z.object({
    ,
    // Image-specific restrictions maxImageDimensions: z.object({
    width: z.number().min(0).default(4096),
    height: z.number().min(0).default(4096)
}).optional(), 
// Processing options autoResize: z.boolean().default(false)  generateThumbnails: z.boolean().default(false),
compressImages;
().default(true),
    // Security options requireSafetyCheck: z.boolean().default(true)  virusScan: z.boolean().default(true),
    // Storage options storageClass: z.enum(['standard', 'cold', 'archive']).default('standard'),
    enableCDN;
z.boolean().default(true),
    // Access options defaultVisibility: z.enum(['public', 'authenticated', 'private']).default('authenticated'),
    signedUrlExpiry;
z.number().min(60).max(7 * 24 * 60 * 60).default(3600); // 1 hour default
;
// Upload Session Schema
export const UploadSessionSchema = z.object({
    id: z.string().min(1),
    // Session details
    userId: z.string().min(1), purpose: FilePurposeSchema,
    // Configuration
    config: UploadConfigSchema
    // Progress tracking
    ,
    // Progress tracking
    totalFiles: z.number().min(0).default(0), completedFiles: z.number().min(0).default(0),
    failedFiles: z.number().min(0).default(0),
    // Upload tracking
    uploadedBytes: z.number().min(0).default(0), totalBytes: z.number().min(0).default(0),
    // Status
    status: z.enum(['active', 'completed', 'failed', 'cancelled']),
    // Files in session files: z.array(z.string()).default([]), // File IDs
    // Error handling errors: z.array(z.object({
...
export const FileAccessLogSchema = z.object({
    ...
    // Access details
    accessedBy: z.string().optional(), // User ID, or null for anonymous accessType: z.enum(['view', 'download', 'thumbnail', 'metadata']),
    // Context
    referrer: z.string().optional(), userAgent: z.string().optional(),
    ipAddress: z.string().optional(),
    // Response
    responseCode: z.number().min(100).max(599), bytesTransferred: z.number().min(0).default(0),
    createdAt: z.date()
});
// File Quota Schema
export const FileQuotaSchema = z.object({
    ...
    maxFiles: z.number().min(0), maxDailyUploads: z.number().min(0),
    // Current usage
    usedStorage: z.number().min(0).default(0), usedFiles: z.number().min(0).default(0),
    ...
    ,
    // Tracking
    lastUpload: z.date().optional(), quotaExceeded: z.boolean().default(false),
    updatedAt: z.date()
});
